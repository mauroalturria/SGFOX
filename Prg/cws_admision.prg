...
Lparameters xtipopac,nrohclinica,mnroadm,mcodcie,mespec,msinreg,leseme,nordeninter,mimedcab
...
Do sp_busco_estados With 57, " and tipo = 51 ", "mwknewAdm"
...
If msinreg=0
	Do cws_registra With minrohclinica
Endif
...
If xtipopac ="INT"
	...
	Do While !Eof() And Inlist(mwklugarinterna.LUG_codsector, 'PQU','PRP','EME','CEG','IDE','IEP')
		Skip 1
	Enddo
	If Eof()
		Go Bottom
...
Endif
If mwknewAdm.estado = 0 AND myip<>'172.16.1.7'
	m1 = Iif(xtipopac="AMB","X","")
	m2 = Iif( xtipopac="INT","X","")
	m3 = Iif(xtipopac="GUA","X","")
Else
	ccm = Iif(mxcentromedico=1,"C",Iif(mxcentromedico=2,"L",Iif(mxcentromedico=3,"P","C")))
	m1 = Iif(xtipopac="AMB",ccm ,"")
	m2 = Iif( xtipopac="INT",ccm ,"")
	m3 = Iif(xtipopac="GUA",ccm ,"")
Endif
