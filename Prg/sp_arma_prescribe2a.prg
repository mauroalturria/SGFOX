...
lparameters mIdevol, mHis,mFecDesde,mFecHasta

mFiltro = ""
mOrden = ""
** ------------------------------Cursor que vamos a devolver
if !used("mwkPrescribe")
	create cursor mwkPrescribe (fecha t,medico C(30),Tipo n(1), estado C(1), Leyenda C(100), Informe M, FecHorBlan t null, MedicoBlan C(30) null, TipoM C(1), Guia I null , Baxter C(10) null, Insumo c(11) )
endif
...
if reccount('mwksolprin2')>0
	if reccount('mwksolprin1')>0
		select * from mwksolprin1 ;
			...
			into cursor mwksolprinia

		select * from mwksolprinia ;
...
if reccount('mwkagregad2')>0
	if reccount('mwkagregad1')>0
		select * from mwkagregad1 ;
			...
			into cursor mwkagregadia

		select * from mwkagregadia ;
			order by PA_FECHORMODIF into cursor mwkagregadi
...
**mfini = Nvl(Ttoc(mwkPsSoluA.PS_fechoraini),'')
				...
				mEvolPrinci = mEvolPrinci + ', VIA ' + mvia + iif( len(mfini) > 1, ', INICIO ' + mfini, '')  + iif(mwkPsSoluA.PS_urgente=1, ' - URGENTE -', '')   &&+ Chr(13)
...
** -------------- Buscamos la fecha-hora de inicializacion para la FECHA.
				...
				mFecHoraBlan = mwkPresFec.pps_fechora
				mMedicoBlan = mwkPresFec.pps_usuariomodif

** -------------- Grabamos registro generado
				if !empty(mEvolPrinci) &&.Or. !Empty(mAgregados)
**mEvolPrinci = mEvolPrinci + mBajaAgre + mAgregados
					insert into mwkPrescribe (fecha,medico,Tipo,estado,Leyenda,Informe,FecHorBlan, MedicoBlan,TipoM,Guia,Baxter,Insumo) ;
						values(mFecHora,mUsuario,mIdTipo,iif(mIdEstado = 1,'A','P'),mleye,mEvolPrinci,mFecHoraBlan,mMedicoBlan,iif(mIdTipo = 3,"I","S"),mb1,mb3,mwkPsSoluA.ps_insumo)
				endif
...
**If Empty(mBajaAgre)
**	mBajaAgre = mBajaAgre + Chr(13) + "(AGREGADOS)" + Chr(10)
**Endif
...
** ----------------- Grabamos registro de baja
							insert into mwkPrescribe (fecha,medico,Tipo,estado,Leyenda,Informe,TipoM,Guia,Baxter,Insumo,FecHorBlan,MedicoBlan) ;
								values(mFecHora,mUsuario,mIdTipo,iif(mIdEstado = 1,'A','P'),mleye,mBajaAgre,"A",mb1,mb3,mpv2,mFecHoraBlan,mMedicoBlan )
...
** ----------------- Grabamos registro Agregado
							insert into mwkPrescribe (fecha,medico,Tipo,estado,Leyenda,Informe,TipoM,Guia,Baxter,Insumo,FecHorBlan,MedicoBlan) ;
								values(mFecHora,mUsuario,mIdTipo,iif(mIdEstado = 1,'A','P'),mleye,mAgregados,"A",mb1,mb3,mpv2,mFecHoraBlan,mMedicoBlan )
...
*!*							mPlanificacion = mPlanificacion + Iif(mwkplanifi.pp_fecpasiva <> Ctod('01/01/1900'), "(BAJA) - ","") + Alltrim(Str(mwkplanifi2.PP_dosis)) + ' ' + Alltrim(mwkplanifi2.PP_unidadvol) + ' '  + Alltrim(mwkplanifi2.PP_frecuencia) + ' ' + mwkplanifi2.PP_urgente
...
*!*					If !Empty(mEvolPrinci) .Or. !Empty(mAgregados)
*!*						mEvolPrinci = mEvolPrinci + mBajaAgre + mAgregados
*!*						Insert Into mwkPrescribe (fecha,medico,Tipo,estado,Leyenda,Informe,FecHorBlan, MedicoBlan) Values(mFecHora,mUsuario,mIdTipo,Iif(mIdEstado = 1,'A','P'),mleye,mEvolPrinci,mFecHoraBlan,mMedicoBlan)
*!*					Endif

				select mwkPsSoluA

			endscan
** --------------- Grabamos el registro Activo generado.
*!*				If !Empty(mEvolPrinci) .Or. !Empty(mAgregados)
*!*					mEvolPrinci = mEvolPrinci + mBajaAgre + mAgregados
*!*					Insert Into mwkPrescribe (fecha,medico,Tipo,estado,Leyenda,Informe) Values(mFecHora,mUsuario,mIdTipo,Iif(mIdEstado = 1,'A','P'),mleye,mEvolPrinci)
*!*				Endif
