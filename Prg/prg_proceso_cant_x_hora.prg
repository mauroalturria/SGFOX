...
do sp_cantidad_turnos_generados with vr_med, vr_fecha, vr_diasem, vr_horad, vr_horah
...
if used('MwkCantxFranja')
	if MwkCantxFranja.totalturnos > 0
		...
		mncant   = Round(((mnsumat/60) * vr_cant),0)
		...
		* Calculo el intervalo de sobreoferta
		...
		mtdura   = datetime(year(vr_fecha),month(vr_fecha),day(vr_fecha),;
							mthoramin,mtduraMin,0)
		mthora_r = datetime(year(vr_fecha),month(vr_fecha),day(vr_fecha),;
							hour(vr_horad),minute(vr_horad),0)
