...
Parameters mabm,mpaciente,mtipopac,mcodmed,mDenPol,mSolDP,mInfFam,mcodCie,mfechaOb,mhoraOb,mobserva,;
	...
	mfetoret,mcamina,msilla, mTurnoEgreso, lnregiob, lbajaygraba
	...
	Else
		If Reccount("mwkrregingr")= 0
			...
			mret =SQLExec(mcon1,"INSERT INTO Tabpacobito(PO_admision, PO_CodMed, PO_Codcie10, PO_DenPol, PO_Estado,PO_FamInf, "+;
				...
				"PO_SolicSillaRueda,PO_Caminando ,PO_TurnoEgreso )"+;
			...
			Else
				If mmotegre =6
					...
					Case mtipopac = "INT"
						If !Used('mwkpacint1')
							Select * From mwkpacint Into Cursor mwkpacint1
						Endif
						Select mwkpacint1
...
*!*							+"' and Nroregistrac = "+ transf(mwkpacint1.pac_codhci)
			...
			*		If !(Between(mdtF,mwksocaux.HoraLLegada ,mwksocaux.HoraFinalizacion ) And mwksocaux.idmotivo = 1)
						...
						If mwkHabSQL.estado = 2
							Do sp_grabo_mesaentsql   With mpac ,mmot,mobs,ment,madmis
						Else
							Do sp_grabo_mesaent  With mpac ,mmot,mobs,ment,madmis

						Endif
			*		Endif
					Do sp_grabo_mesaentaux  With mpac ,mmot,mobs,ment,madmis
...
Case Inlist(mabm,2,3)
	...
	If mid>0
		...
		mret =SQLExec(mcon1,"UPDATE TabPacObito SET PO_Estado = ?mestado, PO_NroCertif = ?mnrocert, "+;
			"PO_MotivoEgreso =?mmotegre, PO_entregadoa =?mentregado, PO_FHEntrega = ?mfecentrega ," +;
			"PO_TurnoEgreso = ?mTurnoEgreso " +mfinal +;
			" WHERE id = ?mid " )
	Else
		mret =SQLExec(mcon1,"INSERT INTO Tabpacobito(PO_admision, PO_CodMed, PO_Codcie10, PO_DenPol, PO_Estado,PO_FamInf, "+;
			" PO_FechaCierre, PO_FechaIngreso, PO_FechaObito, PO_HoraObito, PO_NroCertif, PO_NroSocio, PO_Observac, "+;
			...
			"PO_SolicSillaRueda,PO_Caminando , PO_TurnoEgreso )"+;
			"VALUES (?mpaciente,?mcodmed,?mcodCie,?mDenPol,?mestado,?mInfFam,?mfecnul ,?mdtF,?mfechaOb, ?mhoraOb"+;
			", ?mnrocert ,0,?mobserva, ?mprotocolo,?mSolDP,?mtipopac, ?mfetoret,?mconsmed ,?mmotegre, ?mtipoInt ,"+;
			"?msolicambu,?msilla,?mcamina ,?mTurnoEgreso )")
	Endif
...
Case Inlist(mabm , 4,5) &&& mismo medico modifica todos los datos
	...
	If mid>0
		mret =SQLExec(mcon1,"UPDATE Tabpacobito SET PO_Codcie10 = ?mcodCie, PO_DenPol = ?mDenPol, "+;
			...
			"PO_TurnoEgreso = ?mTurnoEgreso "+;
			" WHERE id = ?mid ")
	Else
		mret =SQLExec(mcon1,"INSERT INTO Tabpacobito(PO_admision, PO_CodMed, PO_Codcie10, PO_DenPol, PO_Estado,PO_FamInf, "+;
			" PO_FechaCierre, PO_FechaIngreso, PO_FechaObito, PO_HoraObito, PO_NroCertif, PO_NroSocio, PO_Observac, "+;
			" PO_Protocolo, PO_SolicDen, PO_TipoPac, PO_Traumatico,PO_ConsMedico ,PO_MotivoEgreso ,PO_TipoInterna, "+;
			"PO_SolicAmbu,PO_SolicSillaRueda,PO_Caminando , PO_TurnoEgreso )"+;
			"VALUES (?mpaciente,?mcodmed,?mcodCie,?mDenPol,?mestado,?mInfFam,?mfecnul ,?mdtF,?mfechaOb, ?mhoraOb, "+;
			"?mnrocert ,0,?mobserva, ?mprotocolo,?mSolDP,?mtipopac, ?mfetoret,?mconsmed ,?mmotegre, ?mtipoInt ,"+;
			"?msolicambu,?g,?mcamina ,?mTurnoEgreso )")
	Endif
