...
*	" codesp in('LABO', 'PSIC', 'FONI') and "
	...
	Select left(ttoc(horatur,2), 5) as hora, left(reg_nombrepac, 40) as reg_nombrepac, ;
		...
		nvl(hca_motfalta,0) as hca_motfalta,nvl(hca_fechaInic,ctot("01/01/1900")) as hca_fechaInic,reg_email, 0 as prereg;
		from mwkphorario1 ;
	...
	Select left(ttoc(horatur,2), 5) as hora, left(reg_nombrepac, 40) as reg_nombrepac, ;
		...
		,0 as hca_motfalta,ctot("01/01/1900") as hca_fechaInic, 1 as prereg;
		from mwkphorario2 ;
		left join mwkpent on codent = ent_codent ;
		left join mwkpesp on codesp = esp_codesp ;
		left join mwkppres on codprest = pre_codprest ;
	...
	Select hora, reg_nombrepac, ;
		ent_descrient, pre_descriprest, codreserva,  reg_nrohclinica, ;
		reg_telefonos, mwkphorario1.usuario, fechatomado, codserv, fechatur, nombre, AFI_nroafiliado, reg_numdocumento, ;
		horatur, id, termina, esp_descripcion, ;
		hdesde1, hhasta1, codesp, codmed, diasem, sala, ;
		codbarra, afiliado, ;
		edad, piso, codprest,;
		hca_motfalta,hca_fechaInic,reg_email, prereg;
	 from mwkphorario3 ;
		union ;
		select hora, reg_nombrepac, ;
		ent_descrient, pre_descriprest, codreserva,  reg_nrohclinica, ;
		...
		codbarra, afiliado, ;
		edad, piso, codprest,;
		hca_motfalta,hca_fechaInic,;
		"NO TIENE" AS reg_email, prereg from mwkphorario4;
	...
	Scan
		...
		If lsigue
			...
			mret = sqlexec(mcon1, "select afiliado,codesp , CODprest " + ;
				"from turnos  " + ;
				"where fechatur < ?mfecturno and codserv <> 1130 and " + ;
				"afiliado = ?mregistracio "+mccpoamb , "mwkconsumos")

			Select * from mwkconsumos;
				where  !inlist (codesp ,'KINE', 'NFII', 'NEUF', 'HOLT', 'ECGR', 'ECIN', 'ECOC', 'ECOO', 'ECOG', 'ECOI', 'ERGO', 'LABO', 'RADI', 'RESO', 'TOMO') ;
				 and  (at('28010', alltrim(str(codprest))) # 1 or codprest = 28010602);
				and at('20012', alltrim(str(codprest)) )# 1 and !inlist(codprest,42010181,78010600,78010601,67010201,22020300);
		...
		If lsigue
			...
			Else
				...
				mret = sqlexec(mcon1, "select afiliado,codesp , CODprest " + ;
					"from turnoshis as turnos  " + ;
					"where &cbusca and codserv <> 1130 and " + ;
					"afiliado = ?mregistracio " +mccpoamb , "mwkconsumos")
		...
		If lsigue
			...
			mret = sqlexec(mcon1, "select HIS_nroregistrac " + ;
				...
				"valesasist " + mcjoinvales +;
				"where HIS_codadmision = VAL_codadmision and " + ;
				"VAL_codsector = 'AMB' and " + ;
				"VAL_codservvale = ser_codserv and " + ;
				"ser_ambulatorio = 'S' and VAL_codservvale <> 1130 and " + ;
				"his_nroregistrac = ?mregistracio "+mbuscoval , "mwkconsumos4")
...
*	    mfec_desde = ctod('31/05/2003')
		...
		mret = sqlexec(mcon1, "select afiliado,sum( case when fechatur >= ?mfec_desde "+;
			...
			"from turnos " + ;
			"where turnos.codesp = 'PSIC' " + mccpoamb+;
			"group by afiliado " + ;
			"", "mwkphoragre1")
...
****
		...
		mret = sqlexec(mcon1, "select afiliado,fechatur " + ;
			...
			" fechatur >= ?mfec_desde "+mccpoamb, "mwkphoragr1")
...
****
	...
	mret = sqlexec(mcon1,"SELECT fecvigend , fecvigenh , horahasta , horadesde , turnos.codmed "+;
		...
		" franjahoraria.codmed = turnos.codmed and (turnos.tipoturno < 8 or turnos.tipoturno >=13)" +mccpoamb+mccpoambf ,"mwkTurNoAsig")
