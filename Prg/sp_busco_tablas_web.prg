*
* Elaboración TXT web
*

*!*	Public mcon1,mcon2,mcon3
*!*	Close databases

*!*	Set DATE TO FRENCH
*!*	Set CENTURY ON
*!*	Set SYSMENU TO
*!*	Set EXCLUSIVE OFF
*!*	Set DECIMALS TO 0
*!*	Set MULTILOCKS ON
*!*	Set NOTIFY OFF
*!*	Set CPDIALOG OFF
*!*	Set SAFETY OFF

*!*	mcon1 = SQLCONNECT("conec01")


Wait "ACTUALIZANDO TABLAS WEB, AGUARDE UNOS MINUTOS" Windows Nowait
If used('mwkespe')
	Use in mwkespe
Endif
mret = sqlexec(mcon1,"select esp_codesp,esp_descripcion,esp_medinterna,esp_matinfantil,esp_clinquir,"+;
	"esp_diagtratamiento "+;
	"from especialid where ESP_Web = 'S' order by esp_codesp","mwkespe")
If mret > 0
	If file('c:\temp\web\especialidad.txt')
		Delete file c:\temp\web\especialidad.txt
	Endif
	If file('c:\temp\web\areas_especialidad.txt')
		Delete file c:\temp\web\areas_especialidad.txt
	Endif
	If used('mwkespe')
		Select mwkespe
		Go top
		If reccount('mwkespe')>0
			Scan
				mcod = mwkespe.esp_codesp
				mdes = mwkespe.esp_descripcion
				mlinea = ''
				mlinea = alltrim(mcod)+'|'+alltrim(mdes)+chr(13)+chr(10)
				Strtofile( mlinea, "c:\temp\web\especialidad.txt", .t. )
				If mwkespe.esp_medinterna = 'S'
					mlinea = ''
					mlinea = '1'+'|'+alltrim(mcod)+chr(13)+chr(10)
					Strtofile( mlinea, "c:\temp\web\areas_especialidad.txt", .t. )
				Endif
				If mwkespe.esp_matinfantil = 'S'
					mlinea = ''
					mlinea = '2'+'|'+alltrim(mcod)+chr(13)+chr(10)
					Strtofile( mlinea, "c:\temp\web\areas_especialidad.txt", .t. )
				Endif
				If mwkespe.esp_clinquir = 'S'
					mlinea = ''
					mlinea = '3'+'|'+alltrim(mcod)+chr(13)+chr(10)
					Strtofile( mlinea, "c:\temp\web\areas_especialidad.txt", .t. )
				Endif
				If mwkespe.esp_diagtratamiento = 'S'
					mlinea = ''
					mlinea = '4'+'|'+alltrim(mcod)+chr(13)+chr(10)
					Strtofile( mlinea, "c:\temp\web\areas_especialidad.txt", .t. )
				Endif
			Endscan
		Endif
		Use in mwkespe
	Endif
Endif
If file('c:\temp\web\prestacion.txt')
	Delete file c:\temp\web\prestacion.txt
Endif
If used('mwkpresta')
	Use in mwkpresta
Endif

*!*	mret = sqlexec(mcon1,"select pre_codprest,pre_descriprest,pre_codservicio,pre_especialidad,"+;
*!*		"CAST( (case when PRE_WebTurnos = 'S' then 1 else 0 end) as int) as turno,Tabmensaje.mensaje as msg "+;
*!*		"from prestacions "+;
*!*		"left join Tabmensaje on codprest=pre_codprest and web='S' " +;
*!*		"left join Tabmensaje on codesp=pre_especialidad and web='S' "+;
*!*		"left join Tabmensaje on codserv=pre_codservicio and web='S' " +;
*!*		"where PRE_fechapasiva is null "+;
*!*		"and PRE_Web = 'S' or PRE_WebTurnos = 'S' "+;
*!*	    "group by pre_codprest "+;
*!*		"order by pre_codprest","mwkpresta")

mret = sqlexec(mcon1,"select pre_codprest,pre_descriprest,pre_codservicio,pre_especialidad,"+;
	"CAST( (case when PRE_WebTurnos = 'S' then 1 else 0 end) as int) as turno,Tabmensaje.mensaje as msg"+;
	" from prestacions"+;
	" left join Tabmensaje on Tabmensaje.codservm = prestacions.pre_codservicio and"+;
	" tabmensaje.web='S'"+;
	" where PRE_fechapasiva is null"+;
	" and PRE_Web = 'S' or PRE_WebTurnos = 'S'"+;
	" order by pre_codprest","mwkpresta")
If mret > 0
	If used('mwkpresta')
		Select mwkpresta
		Go top
		If reccount('mwkpresta')>0
			Scan
				mcod = alltrim(transform(mwkpresta.pre_codprest,"999999999999"))
				mdes = alltrim(mwkpresta.pre_descriprest)
				mtur = transform(mwkpresta.turno,"9")
				mmsg = iif(mwkpresta.turno=1,'Para solicitar un turno a través de la web, para esta práctica haga click'+;
					' sobre la misma',nvl(alltrim(mwkpresta.msg),'.'))
				mlinea = ''
				mlinea = mcod+'|'+mdes+'|'+mtur+'|'+mmsg+chr(13)+chr(10)
				Strtofile( mlinea, "c:\temp\web\prestacion.txt", .t. )
			Endscan
		Endif
		Use in mwkpresta
	Endif
Else
	= aerror(merror)
	Messagebox(merror(1))
Endif

If file('c:\temp\web\empresas.txt')
	Delete file c:\temp\web\empresas.txt
Endif
If used('mwkempre')
	Use in mwkempre
Endif
mret = sqlexec(mcon1,"select ENT_codent,ENT_sigla,ENT_desccomp,ENT_DIRENT,ENT_localent,ENT_url,ENT_email,"+;
	"case when max(ENT_turnosweb)='Y' then 1 else 0 end as ent_turnosweb,"+;
	"case when max(ENT_web)='Y' then 1 else 0 end as ent_web "+;
	"from SQLUser.ENTIDADES "+;
	"where ent_fecpas is null and ( ENT_turnosweb = 'Y' or ENT_web = 'Y' ) "+;
	"order by ENT_desccomp","mwkempre")
If mret > 0
	If used('mwkempre')
		Select mwkempre
		Go top
		If reccount('mwkempre')>0
			Scan all
				mcod1 = mwkempre.ENT_codent
				msig1 = nvl(mwkempre.ENT_sigla,' ')
				mdes1 = nvl(mwkempre.ENT_desccomp,' ')
				mdir1 = nvl(mwkempre.ENT_dirent,' ')
				mloc1 = nvl(mwkempre.ENT_localent,' ')
				murl1 = nvl(mwkempre.ENT_url,' ')
				mail1 = nvl(mwkempre.ENT_email,' ')
				mtur1 = nvl(mwkempre.ENT_turnosweb,' ')
				mweb1 = nvl(mwkempre.ENT_web,' ')
				mcod  = iif(len(alltrim(str(mcod1)))>0,alltrim(str(mcod1)),' ')
				msig  = iif(len(alltrim(msig1))>0,alltrim(msig1),' ')
				mdes  = iif(len(alltrim(mdes1))>0,alltrim(mdes1),' ')
				mdir  = iif(len(alltrim(mdir1))>0,alltrim(mdir1),' ')
				mloc  = iif(len(alltrim(mloc1))>0,alltrim(mloc1),' ')
				murl  = iif(len(alltrim(murl1))>0,alltrim(murl1),' ')
				Mail  = iif(len(alltrim(mail1))>0,alltrim(mail1),' ')
				mtur  = iif(len(alltrim(mtur1))>0,alltrim(mtur1),' ')
				mweb  = iif(len(alltrim(mweb1))>0,alltrim(mweb1),' ')
				mlinea = ''
				mlinea = mcod+'|'+msig+'|'+mdes+'|'+mdir+'|'+mloc+'|'+mail+'|'+murl+'|'+mtur+'|'+mweb+chr(13)+chr(10)
				Strtofile( mlinea, "c:\temp\web\empresas.txt", .t. )
			Endscan
		Endif
		Use in mwkempre
	Endif
Endif

Dimension varea[4]
varea[1]='Medicina Interna'
varea[2]='Materna Infantil'
varea[3]='Cirugía'
varea[4]='Diagnóstico y Tratamiento'

If file('c:\temp\web\areas.txt')
	Delete file c:\temp\web\areas.txt
Endif
For mi = 1 to 4
	mlinea = ''
	mlinea = transform(mi,"9")+'|'+alltrim(varea[mi])+chr(13)+chr(10)
	Strtofile( mlinea, "c:\temp\web\areas.txt", .t. )
Endfor
If file('c:\temp\web\cargos.txt')
	Delete file c:\temp\web\cargos.txt
Endif
If used('mwkcargo')
	Use mwkcargo
Endif

mret = sqlexec(mcon1,"select * from TabCargo where fechapasiva is null","mwkcargo")
If mret > 0
	If used('mwkcargo')

		mlagregar  = ''

		Select mwkcargo
		Go top
		If reccount('mwkcargo')>0
			Scan
				mlinea1 = alltrim(str(mwkcargo.id))+'|'+alltrim(mwkcargo.descrip)+;
					'|'+alltrim(str(mwkcargo.destacado))+'|'+alltrim(str(mwkcargo.nivel))+chr(13)+chr(10)

				If mwkcargo.id = 40
					mlagregar = alltrim(str(mwkcargo.destacado))+'|'+alltrim(str(mwkcargo.nivel))
				Endif

				Strtofile( mlinea1, "c:\temp\web\cargos.txt", .t. )
			Endscan
		Endif
		Use in mwkcargo

*!*			For mip = 1 to 6
*!*				Do case
*!*				Case mip = 1
*!*					mlinea1 = '100'+'|'+'Dirección Administrativa'+'|'+'0'+'|'+'0'+chr(13)+chr(10)
*!*				Case mip = 2
*!*					mlinea1 = '101'+'|'+'Dirección Medica'+'|'+'0'+'|'+'0'+chr(13)+chr(10)
*!*				Case mip = 3
*!*					mlinea1 = '120'+'|'+'Coordinador de Internación, Ambulatorio y Guardia'+'|'+'1'+'|'+'0'+chr(13)+chr(10)
*!*				Case mip = 4
*!*					mlinea1 = '121'+'|'+'Coordinador de Internación y Ambulatorio'+'|'+'1'+'|'+'0'+chr(13)+chr(10)
*!*				Case mip = 5
*!*					mlinea1 = '122'+'|'+'Coordinador de Internación y Guardia'+'|'+'1'+'|'+'0'+chr(13)+chr(10)
*!*				Case mip = 6
*!*					mlinea1 = '123'+'|'+'Coordinador de Ambulatorio y Guardia'+'|'+'1'+'|'+'0'+chr(13)+chr(10)
*!*				Endcase
*!*				Strtofile( mlinea1, "c:\temp\web\cargos.txt", .t. )
*!*			Endfor


**      Agregar cargos  141 al 144 ************** contemplar el ID 40 valores adicionales porner mlagregar

		For mip = 1 to 10

			Do case
			Case mip = 1
				mlinea1 = '100'+'|'+'Dirección Administrativa'+'|'+'0'+'|'+'0'+chr(13)+chr(10)
			Case mip = 2
				mlinea1 = '101'+'|'+'Dirección Medica'+'|'+'0'+'|'+'0'+chr(13)+chr(10)
			Case mip = 3
				mlinea1 = '120'+'|'+'Coordinador de sector Internación, Ambulatorio y Quirúrgico'+'|'+'1'+'|'+'0'+chr(13)+chr(10)
			Case mip = 4
				mlinea1 = '121'+'|'+'Coordinador de sector Internación y Ambulatorio'+'|'+'1'+'|'+'0'+chr(13)+chr(10)
			Case mip = 5
				mlinea1 = '122'+'|'+'Coordinador de sector Internación y Quirúrgico'+'|'+'1'+'|'+'0'+chr(13)+chr(10)
			Case mip = 6
				mlinea1 = '123'+'|'+'Coordinador de sector Ambulatorio y Quirúrgico'+'|'+'1'+'|'+'0'+chr(13)+chr(10)

			Case mip = 7
				mlinea1 = '141'+'|'+'Coordinador de especialidad Ambulatorio'+'|'+mlagregar +chr(13)+chr(10)

			Case mip = 8
				mlinea1 = '142'+'|'+'Coordinador de especialidad Guardia'+'|'+mlagregar +chr(13)+chr(10)

			Case mip = 9
				mlinea1 = '143'+'|'+'Coordinador de especialidad Internación'+'|'+mlagregar +chr(13)+chr(10)

			Case mip = 10
				mlinea1 = '144'+'|'+'Coordinador de especialidad Quirofano'+'|'+mlagregar +chr(13)+chr(10)

			Endcase
			Strtofile( mlinea1, "c:\temp\web\cargos.txt", .t. )
		Endfor

	Endif
Else
	Messagebox("EN ARMADO DEL CURSOR CARGOS",16,"ERROR")
Endif

*!*	hConn = Sqlstringconnect("Driver=MySQL ODBC 3.51 Driver;Server=172.16.2.101;UID=malturria;PWD=mauro;Database=prueba")
*!*	=SQLSETPROP(hConn,"Transactions",2)
*!*	=SQLSETPROP(hConn,"Asynchronous",.F.)

If used('mwkcontenido')
	Use in mwkcontenido
Endif
If used('mwkexporta')
	Use in mwkexporta
Endif
If file('c:\temp\web\contenido.txt')
	Delete file c:\temp\web\contenido.txt
Endif

mret = sqlexec(hConn,"select * from contenido","mwkcontenido")
If mret > 0
	Select transform(id,"999999") as idcod,titulo,especialidad,descripcion,transform(seccion,"9") as idsec,transform(orden,"9") as idord ;
		from mwkcontenido into cursor mwkexporta
	Select mwkexporta
	Go top
	Scan all
		mmemo = STRTRAN(mwkexporta.descripcion, chr(13), '<br>')
		mlines = alines(mvec,mmemo,.t.)
		mtexto = ''
		For mi = 1 to mlines
			mtexto = mtexto + alltrim(mvec[mi]) + ' '
		Endfor
		mc1 = alltrim(mwkexporta.idcod)
		mc2 = alltrim(mwkexporta.titulo)
		mc3 = alltrim(mtexto)
		mc4 = mwkexporta.especialidad
		mc5 = mwkexporta.idsec
		mc6 = mwkexporta.idord
		mlinea = ''
		mlinea = mlinea +mc1+'|'+mc2+'|'+mc3+'|'+mc4+'|'+mc5+'|'+mc6+chr(13)+chr(10)
		Strtofile( mlinea, "c:\temp\web\contenido.txt", .t. )
	Endscan
	If used('mwkexporta')
		Use in mwkexporta
	Endif
	If used('mwkcontenido')
		Use in mwkcontenido
	Endif
Endif

mpaso = .f.
If used('mwkpresta')
	Use in mwkpresta
Endif

*!*	*!* mret = sqlexec(hConn,"select * from prestadores","mwkpresta")
*!*	*mret = sqlexec(mcon1,"select *,id as iDmedico,codesp as codiEsp from prestadores","mwkpresta")

*!*	mret = sqlexec(mcon1,"select prestadores.id as iDmedico,prestadores.nombre,"+;
*!*		"tabprofesp.codiesp,prestadores.emailcorp,especialid.esp_descripcion,"+;
*!*		"tabprofesp.CodCargo,tabcargo.descrip,prestadores.fecaltap,prestadores.fecpas"+;
*!*		"ivap,prestadores.emailcorp,prestadores.codprof,prestadores.sexo,prestadores.codesp as codiEsp"+;
*!*		" from prestadores"+;
*!*		" left join TabProfEsp on prestadores.id = tabprofesp.codprof"+;
*!*		" left join tabcargo on tabprofesp.codcargo = tabcargo.id"+;
*!*		" left join especialid on tabprofesp.codiesp = especialid.esp_codesp"+;
*!*		" where"+;
*!*		" ((dambula = 1 and (prestadores.fecpasiva='1900-01-01' or prestadores.fecpasiva > current_date)"+;
*!*		" and (prestadores.fecalta <= current_date )) or"+;
*!*		" (dguardia = 1 and (prestadores.fecpasivag='1900-01-01' or prestadores.fecpasivag > current_date)"+;
*!*		" and (prestadores.fecaltag <= current_date )) or"+;
*!*		" (dinterna = 1 and (prestadores.fecpasivai='1900-01-01' or prestadores.fecpasivai > current_date)"+;
*!*		" and (prestadores.fecaltai <= current_date )) or"+;
*!*		" (prestadores.fecaltap <= current_date ))"+;
*!*		" and (prestadores.fecpasivap='1900-01-01' or  prestadores.fecpasivap > current_date)"+;
*!*		" and codiesp is not null and codcargo is not null"+;
*!*		" and esp_descripcion is not null"+;
*!*		" and PRESTADORES.web = 'S'"+;
*!*		" and especialid.ESP_Web = 'S'"+;
*!*		" order by prestadores.id,ESP_descripcion,tabprofesp.CodCargo,nombre","mwkpresta")

*!* 07/05/2012

*Set step on

mret = sqlexec(mcon1,"select prestadores.id as IDmedico,prestadores.nombre, tabprofesp.codiesp,especialid.esp_descripcion,"+;
	"tabprofesp.codarea,tabareades.area,tabprofesp.CodCargo,tabcargo.descrip,prestadores.fecaltap,prestadores.fecpasivap,"+;
	"prestadores.cuil, prestadores.matriculas, prestadores.nroproveedor ,prestadores.email, prestadores.fecalta as FechaAmbA,"+;
	"prestadores.fecpasiva as FechaAmbB, prestadores.fecaltag as FechaGuaA, prestadores.fecpasivag as FechaGuaB,"+;
	"prestadores.fecaltai as FechaIntA, prestadores.fecpasivai as FechaIntB, prestadores.lseguro ,"+;
	"(trim(upper(tabcargo.descrip)) || ' ' || trim(upper(tabareades.area))) as campo1,"+;
	"(case when trim(upper(tabareades.area)) = 'GUARDIA' THEN 'GUAR' ELSE codiesp END ) as campo2,"+;
	"(case when trim(upper(tabareades.area)) = 'GUARDIA' THEN 'GUARDIA MEDICA (EMERGENCIAS MEDICAS)' "+;
	" ELSE tabareades.area END ) as campo3,tabcargo.orden"+;
	" from prestadores"+;
	" left join TabProfEsp on prestadores.id=tabprofesp.codprof"+;
	" left join tabcargo on tabprofesp.codcargo = tabcargo.id "+;
	" left join especialid on tabprofesp.codiesp = especialid.esp_codesp"+;
	" left join tabareades on tabprofesp.codarea = tabareades.id"+;
	" where"+;
	" ( ( dambula = 1 and (prestadores.fecpasiva='1900-01-01' or prestadores.fecpasiva >current_date)"+;
	" and (prestadores.fecalta <= current_date ) ) or ( dguardia = 1 and (prestadores.fecpasivag='1900-01-01'"+;
	" or prestadores.fecpasivag > current_date) and (prestadores.fecaltag <= current_date )  )"+;
	" or ( dinterna = 1 and (prestadores.fecpasivai='1900-01-01'"+;
	" or prestadores.fecpasivai > current_date) and (prestadores.fecaltai <= current_date ) )"+;
	" or (prestadores.fecaltap <= current_date ))"+;
	" and (prestadores.fecpasivap='1900-01-01' or  prestadores.fecpasivap > current_date) "+;
	" and esp_descripcion is not null"+;
	" and PRESTADORES.web = 'S'"+;
	" and especialid.ESP_Web = 'S'"+;
	" and tabcargo.staff = 2 "+;
	" group by prestadores.id,tabprofesp.codarea order by esp_descripcion,nombre,codcargo,tabprofesp.codarea","mwkpresta")

	

*
* 07/05/2012
* Cargos ( Coordinadores / Subcoordinadores )
*  and CodCargo in (20,21,25,26,42,40)
*

*!*	" (( dambula = 1 and (prestadores.fecpasiva='1900-01-01' or prestadores.fecpasiva > current_date))"+;
*!*	" or ( dguardia = 1 and (prestadores.fecpasivag='1900-01-01' or prestadores.fecpasivag > current_date))"+;
*!*	" or ( dinterna = 1 and (prestadores.fecpasivai='1900-01-01' or prestadores.fecpasivai > current_date))"+;
*!*	" or (prestadores.fecpasivap='1900-01-01' or  prestadores.fecpasivap > current_date)"+;
*!*	" or (prestadores.fecalta >= current_date ) )"+;
*!*	" and codiesp is not null and codcargo is not null"+;
*!*	" and esp_descripcion is not null"+;
*!*	" and PRESTADORES.web = 'S'"+;
*!*	" and especialid.ESP_Web = 'S'"+;
*!*	" order by prestadores.id,ESP_descripcion,tabprofesp.CodCargo,nombre","mwkpresta")
*!* *!* " group by prestadores.id"+;
*!*	select mwkpresta
*!*	go top
*!*	brow

If mret >0

	Select IDmedico,nombre,codiesp,esp_descripcion,codarea,area,CodCargo,descrip,fecaltap,fecpasivap,;
		cuil,matriculas,nroproveedor,email,FechaAmbA,FechaAmbB,FechaGuaA,FechaGuaB,;
		FechaIntA,FechaIntB,lseguro,campo1,campo2,campo3,orden,;
		fcargo(CodCargo,CodArea,1,'') as CodCargoN, fcargo(CodCargo,codarea,2,descrip) as descripN;
		from mwkpresta into cursor mwkpresta
		
	Select IDmedico,nombre,codiesp,esp_descripcion,codarea,area,CodCargoN as CodCargo,descripN as descrip,fecaltap,fecpasivap,;
		cuil,matriculas,nroproveedor,email,FechaAmbA,FechaAmbB,FechaGuaA,FechaGuaB,;
		FechaIntA,FechaIntB,lseguro,campo1,campo2,campo3,orden;
		from mwkpresta into cursor mwkpresta	
		
		
*	set step on	
		
*!*		select mwkpresta
*!*		go top
*!*		copy to c:\mwkpresta2 foxplus
*!*		*xl5		
				
	If used('mwkcoord1')
		Use in mwkcoord1
	Endif

	mret = sqlexec(hConn,"select * from TabCoord order by TCO_especialidad","mwkcoord1")
	If mret > 0
		If used('mwkcoord2')
			Use in mwkcoord2
		Endif
		mret = sqlexec(hConn,"select * from TabSubCoord order by TSO_especialidad","mwkcoord2")
		If mret > 0
			mpaso = .t.
		Endif
	Endif

Endif

If mpaso

	If file('c:\temp\web\personas.txt')
		Delete file c:\temp\web\personas.txt
	Endif
	If file('c:\temp\web\personas_cargo.txt')
		Delete file c:\temp\web\personas_cargo.txt
	Endif
	If used('mwkexporta')
		Use in mwkexporta
	Endif
	If used('mwkexporta1')
		Use in mwkexporta1
	Endif


*!*		Select * from mwkpresta ;
*!*			left join mwkcoord1 on IDmedico = TCO_idMed ;
*!*			left join mwkcoord2 on IDmedico = TSO_idMed ;
*!*			order by IDmedico ;
*!*			where len(alltrim(nombre)) > 0;
*!*			group by codiEsp, CodCargo, IDmedico into cursor mwkexporta1


*	Set step on

	Select *, alltrim(campo2) as campog2 from mwkpresta ;
		left join mwkcoord1 on IDmedico = TCO_idMed ;
		left join mwkcoord2 on IDmedico = TSO_idMed ;
		order by IDmedico ;
		where len(alltrim(nombre)) > 0;
		into cursor mwkexporta1


	Select * from mwkexporta1 ;
		group by campog2, CodCargo, IDmedico into cursor mwkexporta1


*!*		select mwkexporta1
*!*		go top
*!*		copy to c:\mwkexporta12 foxplus
*!*		*xl5
	

	Select * from mwkexporta1 group by IDmedico into cursor mwkexporta

	Select mwkexporta
	Go top

	Scan all

*!*			If (mwkexporta.TCO_registro <> 'REG0035' and mwkexporta.TCO_registro <> 'REG0036' and ;
*!*					mwkexporta.TCO_registro <> 'REG0009' and mwkexporta.TCO_registro <> 'REG0027') or ;
*!*					( nvl(mwkexporta.TCO_registro,'1')='1' )

		midmed = alltrim(transform(mwkexporta.IDmedico,"999999"))
		mbusid = mwkexporta.IDmedico
		mbusca = 0

		If used('mwkprestaM')
			Use in mwkprestaM
		Endif

		mret = sqlexec(mcon1,"select * from prestadores where id=?mbusid","mwkprestaM")
		If mret > 0
			If used('mwkprestaM')
				If reccount('mwkprestaM')>0
					Select mwkprestaM
					Go top
					mbusca = mwkprestaM.codprof
					mlsexo = mwkprestaM.sexo
					memail = alltrim(nvl(mwkprestaM.emailcorp,'')) && '.'
				Endif
			Endif
		Endif

		If used('mwkprotit')
			Use in mwkprotit
		Endif

*!*     mret = sqlexec(hConn,"select * from TabProfesion where id=?mbusca","mwkprotit")
		mret = sqlexec(mcon1,"select * from TabProfesion where id=?mbusca","mwkprotit")
		If mret > 0
			If used('mwkprotit')
				If reccount('mwkprotit')>0
					If mlsexo = 'M' &&mwkexporta.sexo = 'M'
						mltitulo = alltrim(mwkprotit.titulo)
					Else
						mltitulo = alltrim(mwkprotit.titulof)
					Endif
				Endif
			Endif
		Endif
		If mbusca>0
			mnombr = mltitulo+' '+alltrim(mwkexporta.nombre)
		Else
			mnombr = mltitulo+' '+alltrim(mwkexporta.nombre)
		Endif
		If nvl(mwkexporta.TSO_registro,'0') <> '0'
*!**!*      memail = alltrim(nvl(mwkexporta.TSO_email,'.'))
			mv1 = alltrim(nvl(mwkexporta.TSO_actividad2  ,'')) && '.'
			mv2 = alltrim(nvl(mwkexporta.TSO_actlab      ,'')) && '.'
			mv3 = alltrim(nvl(mwkexporta.TSO_organismos  ,'')) && '.'
			mv4 = alltrim(nvl(mwkexporta.TSO_formacion   ,'')) && '.'
			mv5 = alltrim(nvl(mwkexporta.TSO_investiga   ,'')) && '.'
			mv6 = alltrim(nvl(mwkexporta.TSO_cursos      ,'')) && '.'
			mv7 = alltrim(nvl(mwkexporta.TSO_docente     ,'')) && '.'
			mv8 = alltrim(nvl(mwkexporta.TSO_especialidad,'')) && '.'
		Else
*!**!*         memail = alltrim(nvl(mwkexporta.TCO_email,'.'))
			mv1 = alltrim(nvl(mwkexporta.TCO_actividad2  ,'')) && '.'
			mv2 = alltrim(nvl(mwkexporta.TCO_actlab      ,'')) && '.'
			mv3 = alltrim(nvl(mwkexporta.TCO_organismos  ,'')) && '.'
			mv4 = alltrim(nvl(mwkexporta.TCO_formacion   ,'')) && '.'
			mv5 = alltrim(nvl(mwkexporta.TCO_investiga   ,'')) && '.'
			mv6 = alltrim(nvl(mwkexporta.TCO_cursos      ,'')) && '.'
			mv7 = alltrim(nvl(mwkexporta.TCO_docente     ,'')) && '.'
			mv8 = alltrim(nvl(mwkexporta.TCO_especialidad,'')) && '.'
		Endif
		mv1a = iif(len(alltrim(mv1))>1,mv1,'') && '.'
		mv1  = mv1a
		mv1a = strtran(mv1 ,chr(10),'')
		mv1b = strtran(mv1a,chr(13),'<br>')
		mv1  = mv1b

		mv2a = iif(len(alltrim(mv2))>1,mv2,'') && '.'
		mv2  = mv2a
		mv2a = strtran(mv2 ,chr(10),'')
		mv2b = strtran(mv2a,chr(13),'<br>')
		mv2  = mv2b

		mv3a = iif(len(alltrim(mv3))>1,mv3,'') && '.'
		mv3  = mv3a
		mv3a = strtran(mv3 ,chr(10),'')
		mv3b = strtran(mv3a,chr(13),'<br>')
		mv3  = mv3b

		mlinea1 = ''

		If len(alltrim(mv1))>1
			mlinea1 = mlinea1+'En esta Institución :<br>'+mv1
		Endif
		If len(alltrim(mv2))>1
			mlinea1 = mlinea1+'<br>Asistencial y Laboral :<br>'+mv2
		Endif
		If len(alltrim(mv3))>1
			mlinea1 = mlinea1+'<br>En Organismos Científicos :<br>'+mv3
		Endif

		mlinea1a = strtran(mlinea1,'|','')

		mlinea1 = mlinea1a

		mv4a = iif(len(alltrim(mv4))>1,mv4,'') && '.'
		mv4  = mv4a
		mv4a = strtran(mv4 ,chr(10),'')
		mv4b = strtran(mv4a,chr(13),'<br>')
		mv4  = mv4b
		mv5a = iif(len(alltrim(mv5))>1,mv5,'') && '.'
		mv5  = mv5a
		mv5a = strtran(mv5 ,chr(10),'')
		mv5b = strtran(mv5a,chr(13),'<br>')
		mv5  = mv5b
		mv6a = iif(len(alltrim(mv6))>1,mv6,'') && '.'
		mv6  = mv6a
		mv6a = strtran(mv6 ,chr(10),'')
		mv6b = strtran(mv6a,chr(13),'<br>')
		mv6  = mv6b
		mv7a = iif(len(alltrim(mv1))>1,mv7,'') && '.'
		mv7  = mv7a
		mv7a = strtran(mv7 ,chr(10),'')
		mv7b = strtran(mv7a,chr(13),'<br>')
		mv7  = mv7b

		mlinea2 = midmed+'|'+mnombr+'|'+memail+'|'+mlinea1 +'|'+mv4+'|'+mv5+'|'+mv6+'|'+mv7+chr(13)+chr(10)

		Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*		Endif
	Endscan

*!*		For mip = 1 to 2
*!*			If mip = 1
*!*				midmed = '5000'
*!*				mnombr = 'Dr. GUTSZTAT JORGE'
*!*				memail = 'direccion@silver-cross.com.ar'
*!*			Else
*!*				midmed = '5001'
*!*				mnombr = 'Dr. PEZZELLA HECTOR'
*!*				memail = 'direccion@silver-cross.com.ar'
*!*			Endif
*!*			mlinea1 = ''
*!*			mv4  = ''
*!*			mv5  = ''
*!*			mv6  = ''
*!*			mv7  = ''
*!*			mlinea2 = midmed+'|'+mnombr+'|'+memail+'|'+mlinea1 +'|'+mv4+'|'+mv5+'|'+mv6+'|'+mv7+chr(13)+chr(10)


	mlinea2 = '5000|Cr. JORGE GUTSZTAT|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5001|eMail|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5002|Dr. NESTOR MORGULIS|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5003|Lic. EDUARDO GISMONDI|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5004|Lic. DANIEL DEVIA|personal@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5005|Sra. CARMEN GIL|facturacion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5006|Lic. MIRTA BARRAZA|ambulatorio@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5007|Lic. VIVIANA DIAZ|sistemas@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*	mlinea2 = '5008|Sra. MA. PIA DAMIANI|compras@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
*!* mlinea2 = '5025|Lic. JOHANNA MOISES|jmoises@silver-cross.com.ar|||||'+chr(13)+chr(10)

*!*	mlinea2 = '5026|Lic. JOHANNA MOISES|compras@silver-cross.com.ar|||||'+chr(13)+chr(10)
	
	mlinea2 = '5026|Lic. SILVANA PANZA|compras@silver-cross.com.ar|||||'+chr(13)+chr(10)
	
	
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5009|Lic. PATRICIA CAPURRO|hoteleria@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5010|Ing. CARLOS DEVOTO|mantenimiento@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5011|Dra. ESTELA IZQUIERDO|farmacia@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5012|Cr. DANIEL PEREZ|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*	mlinea2 = '5013|Lic. ANGELA SARLI|enfermeria@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

	mlinea2 = '5014|Dg. VIVIANA ARGENTIERI|diseno@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5015|Estudio COFFEN & LEBELLOT|arquitectura@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*	mlinea2 = '5016|Dr. MARCELO GRECO|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*		mlinea2 = '5017|Dr. ROBERTO YJILIOFF|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*		Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
*!*		mlinea2 = '5018|Dra. LILIANA HOJMAN|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*		Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*	mlinea2 = '5019|Dr. DIEGO ARANOVICH|ambulatorio@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
*!*	mlinea2 = '5020|Dr. LEONARDO GELER|ambulatorio@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*		mlinea2 = '5024|Dra. ADRIANA RUSSO|arusso@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*		Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*		mlinea2 = '5021|Dr. JULIO MATZ|ambulatorio@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*		Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )
	mlinea2 = '5022|Tec. NORA RODRIGUEZ|cirugia@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*	mlinea2 = '5023|Lic. ANGELA SARLI|enfermeria@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*		mlinea2 = '5025|Lic. ALEJANDRO SANTOS|enfermeria@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*		Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )


	mlinea2 = '5027|Dra. MA. ISABEL BECONI|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*		mlinea2 = '5028|Dr.ROBERTO FROMENT|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
*!*		Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

	mlinea2 = '5029|Dr. EDUARDO ESKENAZI|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

	mlinea2 = '5030|Dr. GUSTAVO CASTAGNETO|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

	mlinea2 = '5031|Dr. LUIS GONZALEZ|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

	mlinea2 = '5032|Dr. GUSTAVO TISMINETZKY|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

*!*	mlinea2 = '5033|Lic. MABEL OVIEDO|enfermeria@silver-cross.com.ar|||||'+chr(13)+chr(10)

	mlinea2 = '5033|Lic. SILVINA ROJAS|enfermeria@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )

	mlinea2 = '5034|Dr. MANUEL KLEIN|direccion@silver-cross.com.ar|||||'+chr(13)+chr(10)
	Strtofile( mlinea2, "c:\temp\web\personas.txt", .t. )


*!*	Endfor

*!* 07-10-2011 VISTO CON NICOLAS (MAIL) NUEVOS CARGOS

	Dimension vpas[3]
	vpas[1]=.f.
	vpas[2]=.f.
	vpas[3]=.f.

	Select mwkexporta1
	Go top
	Do while !eof()

		If used('mwkexporta2')
			Use in mwkexporta2
		Endif

		midmev   = mwkexporta1.IDmedico
*		mcodesp1 = mwkexporta1.codiEsp

		mcodesp1 = alltrim(mwkexporta1.campo2)

*!*		22	Coordinador de sector Internación
*!*		23	Coordinador de sector Ambulatorio
*!*		24	Coordinador de sector Quirúrgico

		midmed = alltrim(transform(mwkexporta1.IDmedico,"999999"))
		Select * from mwkexporta1 where IDmedico=midmev And codiesp = mcodesp1 into cursor mwkexporta2

		Select mwkexporta2
		Go top

		Scan all

*!*				If mwkexporta2.CodCargo = 2 or mwkexporta2.CodCargo = 22  && Internación
*!*					mv8b    = alltrim(mwkexporta2.codiEsp)
*!*					vpas[1] =.t.
*!*				Endif

*!*				If mwkexporta2.CodCargo = 3 or mwkexporta2.CodCargo = 23 && Ambulatorio
*!*					mv8b    = alltrim(mwkexporta2.codiEsp)
*!*					vpas[2] =.t.
*!*				Endif

*!*				If mwkexporta2.CodCargo = 4 or mwkexporta2.CodCargo = 24 && Guardia
*!*					mv8b    = alltrim(mwkexporta2.codiEsp)
*!*					vpas[3] =.t.
*!*				Endif

			If mwkexporta2.CodCargo = 22  && Internación
*				mv8b   = alltrim(mwkexporta2.codiEsp)
				mv8b    = alltrim(mwkexporta2.campo2)
				vpas[1] =.t.
			Endif

			If mwkexporta2.CodCargo = 23 && Ambulatorio
*				mv8b   = alltrim(mwkexporta2.codiEsp)
				mv8b    = alltrim(mwkexporta2.campo2)
				vpas[2] =.t.
			Endif

			If mwkexporta2.CodCargo = 24 && Quirúrgico
*				mv8b   = alltrim(mwkexporta2.codiEsp)
				mv8b    = alltrim(mwkexporta2.campo2)
				vpas[3] =.t.
			Endif

			If  mwkexporta2.CodCargo <> 22 and mwkexporta2.CodCargo <> 23 and mwkexporta2.CodCargo <> 24

*				If mwkexporta2.CodCargo = 1 or mwkexporta2.CodCargo > 4

				mcargo  = alltrim(transform(mwkexporta2.CodCargo,"9999"))
*				mv8    = alltrim(mwkexporta2.codiEsp)
				mv8     = alltrim(mwkexporta2.campo2)


				mlinea3 = midmed+'|'+mcargo+'|'+mv8+chr(13)+chr(10)
				Strtofile( mlinea3, "c:\temp\web\personas_cargo.txt", .t. )

*				Endif

			Endif

		Endscan
		mpaso2 = .f.

		Do case
		Case vpas[1] and !vpas[2] and !vpas[3]
			mpaso2=.t.
*			mcargo='2'
			mcargo='22'

		Case !vpas[1] and vpas[2] and !vpas[3]
			mpaso2=.t.
*			mcargo='3'
			mcargo='23'

		Case !vpas[1] and !vpas[2] and vpas[3]
			mpaso2=.t.
*			mcargo='4'
			mcargo='24'

		Case vpas[1] and vpas[2] and vpas[3]
			mpaso2 = .t.
			mcargo = '120'

		Case vpas[1] and vpas[2] and !vpas[3]
			mpaso2 = .t.
			mcargo = '121'

		Case vpas[1] and !vpas[2] and vpas[3]
			mpaso2 = .t.
			mcargo = '122'

		Case !vpas[1] and vpas[2] and vpas[3]
			mpaso2 = .t.
			mcargo = '123'

		Endcase

		If mpaso2
			mlinea3 = midmed+'|'+mcargo+'|'+mv8b+chr(13)+chr(10)
			Strtofile( mlinea3, "c:\temp\web\personas_cargo.txt", .t. )
		Endif

		vpas[1]=.f.
		vpas[2]=.f.
		vpas[3]=.f.

		Select mwkexporta1
*		Do while !eof() and mwkexporta1.IDmedico = midmev And mwkexporta1.codiEsp = mcodesp1
		Do while !eof() and mwkexporta1.IDmedico = midmev And alltrim(mwkexporta1.campo2) = mcodesp1
			Skip
		Enddo

	Enddo
	Release vpas

	If used('mwkexporta2')
		Use in mwkexporta2
	Endif
*!*
*!*
*!*

	For mip = 1 to 2
		mv8 = 'CLIN'
		If mip = 1
			midmed = '5000'
			mcargo = '100'
*!*			mnombr = 'Dr. GUTSZTAT JORGE'
		Else
			midmed = '5001'
			mcargo = '101'
*!*			mnombr = 'Dr. PEZZELLA HECTOR'
		Endif
		mlinea3 = midmed+'|'+mcargo+'|'+mv8+chr(13)+chr(10)
		Strtofile( mlinea3, "c:\temp\web\personas_cargo.txt", .t. )
	Endfor
	If used('mwkexporta')
		Use in mwkexporta
	Endif
	If used('mwkexporta1')
		Use in mwkexporta1
	Endif
	If used('mwkpresta')
		Use in mwkpresta
	Endif
	If used('mwkcoord1')
		Use in mwkcoord1
	Endif
	If used('mwkcoord2')
		Use in mwkcoord2
	Endif
Else
	Messagebox("EN ARMADO DE ARCHIVO PERSONAS",16,"ERROR")
Endif

*!*	Close databases
*!*	= SQLDisconnect(mcon1)
*!*	= SQLDisconnect(hConn)

*!*	Messagebox("ARCHIVOS TXT, TABLAS ELABORADOS",48,"WEB")

Return
** 

Function fcargo(v1,v2,v3,v4)

	If v3 = 1 && Codigo

		mretorno = v1
		If v1 = 40
			Do case
			Case v2 = 1
				mretorno = 141
			Case v2 = 2
				mretorno = 142
			Case v2 = 3
				mretorno = 143
			Case v2 = 4
				mretorno = 144
			Endcase
		Endif
		
	Else && Descripcion

		mretorno = v4
		If v1 = 40
			Do case
			Case v2 = 1
				mretorno = 'Coordinador de especialidad Ambulatorio'
			Case v2 = 2
				mretorno = 'Coordinador de especialidad Guardia'
			Case v2 = 3
				mretorno = 'Coordinador de especialidad Internación'
			Case v2 = 4
				mretorno = 'Coordinador de especialidad Quirofano'
			Endcase
		Endif

	Endif

Return mretorno
