...
ENDIF
If !Empty(micuit)
	mRet = SQLExec(mcon1,"update registracio set REG_cuit = ?micuit "+;
	"where reg_nrohclinica = '"+Alltrim(xnrohclinica)+"'") 
endif
...
mbusco1cw = "where reg_nrohclinica = '"+Alltrim(xnrohclinica)+"' and "
If Vartype(msql_reg)<>"C"
	msql_reg=""
Endif
msql_reg_ant = msql_reg
Do sp_busco_nombre_paciente_1 With mbusco1cw, 1, '',,"mwkcswpac"
...
cmail =Chrtran(cmail , "§$%/()=?`'!><;:_#*+_", "")
cmail =	IIF(At("@",cmail)=0,'',cmail)
*!*		Else
