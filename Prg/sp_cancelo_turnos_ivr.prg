...
*!*		"afiliado>2 and usuario = 'TURNOSWEB' and tipoturno not in (0,5,7,14,15,16,17,18,19,20) order by HORATUR ", "mwktodos")
*!*	 
*!*		miarchivo = 'mafrl.txt'
	...
	If mwktodos.afiliado>1
		If  mwktodos.fechatur = mifecha
			...
			mret = SQLExec(mcon1, "insert into turnoscancel(afiliado, codcancela, codent, codesp, " + ;
				...
				"observa, codserv,UsuarioSector,idturnos &mcicpoamb,ambcentro ) " + ;
				...
				" ?mobserva, ?mcodserv, ?mususec, ?midtur &mvicpoamb,?mxcentromedico)")
			...
			mret = SQLExec(mcon1, "insert into turnos(afiliado, codesp, codmed, codprest, codserv,"+;
				...
				" tipotomado, tipoturno, solicigia, usuario, observa, fechagenera,usuariogenera,UsuarioSector,codprest &mcicpoamb,ambcentro) " + ;
				"values(0, '', ?mcodmed, 0, 0, 0, 0,'', ?mdiasem, ?mfectur, ?mhhmmTur, ?mhortur, 0, 0, " + ;
				"?mturno, 0, ?musuari1, ' ',?mfgenera,?musugen,0,?mnucodprest  &mvicpoamb,?mxcentromedico)")
			If mret < 0
				Messagebox("ERROR EN LA ACTUALIZACION DEL ARCHIVO DE TURNOS, AVISAR A SISTEMAS",16, "Validacion")
