...
Parameters mparam
If vartype(mparam)="C"
	musua 	= Left(mparam,At(",",mparam)-1)
	mparam 	= Substr(mparam,At(",",mparam)+1)
	If At(",",mparam)>0
		msector = Left(mparam,At(",",mparam)-1)
		mparam 	= Substr(mparam,At(",",mparam)+1)
		minreg	= mparam
	Else
		msector = mparam
...
If !Used("mwkserver1")
	lDisconnec = .T.
	Do Sp_Conexion With "AUTORPRINT"
Endif
...
If Vartype(msector)#"N"
	msector = mwksector2.Id
Endif
...
Do Form frmautor22 With Val(Transform(minreg))
Read Events
If 	lDisconnec
	Do sp_desconexion
Endif
