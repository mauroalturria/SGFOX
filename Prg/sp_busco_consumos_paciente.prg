...
mret = SQLExec(mcon1, "select VAL_codadmision, VAL_tipopaciente, VAL_fechasolicitud, VAL_horasolicitud, " + ;
	...
	",VAL_observaciones,"+;
	"val_codpun,VAL_OperadorCarga,val_nrointfactura,VAL_estado,PAC_codhci ,pac_centromedico "+;
	"from pacientes, valesasist,coberturas " + ;
	"where PAC_codadmision = VAL_codadmision and PAC_codadmision = COB_pacientes and " +mfitraent + ;
...
mfitraent = IIF(Empty(cEntidades ),''," cob_codentidad in ( "+cEntidades +") and ")
	...
	mret = SQLExec(mcon1, "select vaq_codadmision, VAQ_tipopaciente, VAQ_fechasolicitud, vaq_horacomienzo , " + ;
		...
		",VAQ_observaciones,"+;
		"vaq_codpunq,PAC_codhci,pac_centromedico  "+;
...
mfitraent = IIF(Empty(cEntidades ),''," HIS_codentidad in ( "+cEntidades +") and ")
	...
	mret = SQLExec(mcon1, "select vaq_codadmision, vaq_tipopaciente, vaq_fechasolicitud, vaq_horacomienzo, " + ;
		"vaq_codpunq, vaq_protocolo, PAC_sectorinternac,PAC_cama,PAC_habitacion, vaq_medicocabecera,vaq_codservvale " + ;
		",VAQ_observaciones,vaq_codpunq,PAC_codhci,pac_centromedico  "+;
	...
	Select vaq_codadmision As val_codadmision, vaq_tipopaciente As val_tipopaciente ;
		...
		,Left(VAQ_observaciones,35) As VAL_observaciones,vaq_codpunq As val_codpun,1 As VAL_OperadorCarga,1 As val_nrointfactura;
		,3 As VAL_estado,PAC_codhci,pac_centromedico ;
		,2 As tv ;
		...
		,pac_sectorinternac,PAC_cama,PAC_habitacion,Val(vaq_medicocabecera) As val_prestador ;
		,vaq_codservvale As val_codservvale ;
		,Left(VAQ_observaciones,35) As VAL_observaciones,vaq_codpunq As val_codpun,1 As VAL_OperadorCarga,;
		1 As val_nrointfactura,3 As VAL_estado,PAC_codhci,pac_centromedico ;
		,2 As tv ;
...
msql_cons = "select VAL_codadmision, VAL_tipopaciente, VAL_fechasolicitud, " + ;
	...
	" VAL_observaciones,val_codpun,VAL_horasolicitud ,VAL_OperadorCarga,val_nrointfactura,VAL_estado,"+;
	" PAC_codhci,NVL(pac_centromedico,1) as  pac_centromedico,_selection "+;
	" from mwkconsumoss "+;
