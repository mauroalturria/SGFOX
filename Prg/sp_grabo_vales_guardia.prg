...
***
mret = sqlexec(mcon1,"select fechahoraate from TabPlanGua where id =1 ","mwkontrol")
mfecdes = ttod(mwkontrol.fechahoraate )-2
mfechas = date()
...
if mret < 0
	...
	if mret < 0
		...
		mret = sqlexec(mcon1, "select VAL_codvaleasist, VAL_codservvale, COB_codentidad, ENT_descrient, " + ;
			"ser_descripserv, VAL_prestador ,nombre " + ;
		...
		if mret < 0
			...
			mtime4 = seconds()
			mret = sqlexec(mcon1, "select nrovale,guardia.fechahoraate,guardiavale.diagnostico,guardia.protocolo  " + ;
				"from guardia,guardiavale  " + ;
			...
			if mret < 0
				...
				select mwktodogua1.VAL_codvaleasist, PAC_nombrepaciente, PAC_codhce, 9, ;
					...
					pia_codprest, pia_cantsolicitada, pre_descriprest, VAL_codservvale, VAL_prestador, ;
...
endscan
mret = sqlexec(mcon1,"update TabPlanGua set fechahoraate = ?mfechastop where id =1 ")
**order by VAL_fechasolicitud, VAL_horasolicitud