...
IF VARTYPE(mbusco)#"C"
	mbusco=''
ENDIF
DO sp_busco_pac_internados_red
mret = sqlexec(mcon1, "select VAL_codvaleasist,VAL_fechasolicitud,VAL_horasolicitud,PIA_codprest,PAC_habitacion, PAC_cama,"+;
	...
	" PRE_especialidad, ESP_descripcion, VAL_observaciones, tabcie10.descrip as diagnosticoCie, pacientes.PAC_descripdiagn "+;
	...
	" where val_codservvale <> 5410 and  Presinsuvas.PIA_VALESASIST = Valesasist.VALESASIST and val_estado = 2  "+;
	" and VAL_fechasolicitud>= ?mfec "+mbusco,"mwkvalesfar0")
...
select VAL_codvaleasist,VAL_fechasolicitud,VAL_horasolicitud,VAL_URGENCIASERV,;
	...
	TVO_Obser ,tvo_fechaestudio, PRE_especialidad, ESP_descripcion, VAL_observaciones, diagnosticoCie, PAC_habitacion, PAC_cama,PAC_descripdiagn ;
	from mwkvalesfar0 WHERE pac_codhci in (SELECT pac_codhci FROM mwkpacact );
...
select PAC_nombrepaciente,"GUARD" as PAC_habitacion, nvl(PAC_cama,SPACE(3)) as PAC_cama,;
	...
	from mwkconsumoss ;
	order by PAC_nombrepaciente,VAL_fechasolicitud desc into cursor mwkconsu01g


select *,left(scamhab1,14) as scamhab,;
	substr(scamhab1,18) as sfecha from mwkconsu01g into cursor mwkconsumog
