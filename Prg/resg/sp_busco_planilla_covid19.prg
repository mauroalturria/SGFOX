   ...
   Case tcTipo = 3  						&& tcTipo = 3 (Todas )    	/ tcBuscar = desde hasta fecha
      ...
      *		 Buscamos Guardia por Fecha
      ...
      TEXT TO lcCMD_SQL TEXTMERGE NOSHOW PRETEXT 7
	      ...
	      From
	      Guardia Guardia
	      INNER Join ZabFichePnCov19 ZabFichePnCov19 On  ZabFichePnCov19.COV_Registrac = Guardia.nroregistrac
            WHERE Guardia.FechahoraIng >= ?m.ldFechaDesde AND Guardia.FechahoraIng <= ?m.ldFechaHasta
	      Order By Guardia.Id Desc
      ENDTEXT
      ...
      *  Buescamos Ambulatorio por fecha
      ...
      FROM Tabambulatorio Tabambulatorio
      INNER JOIN Zabfichepncov19 Zabfichepncov19 ON  Zabfichepncov19.COV_Registrac = Tabambulatorio.nroregistrac
		 WHERE  Tabambulatorio.fechahoraing >= ?m.ldFechaDesde AND Tabambulatorio.FechahoraIng <= ?m.ldFechaHasta
