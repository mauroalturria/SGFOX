...
if mfecdes <= mfechalimite
	mret = sqlexec(mcon1, "select turnos.id,afiliado, fechatur, horatur, codmed, nombre,turnos.codesp, " + ;
		...
		"  and turnos.codserv<>7000 " + mbusco3 , "mwktodosc2a")
...
mret =	sqlexec(mcon1, "select pre_codprest,pre_especialidad,ESP_codesp, " + ;
	"PRE_agendaturnos, ESP_descripcion,ESP_cantsinturno ,PRE_modoatencion  " + ;
...
If mfechas < ctod("04/09/2022") &&&inicio Lima
	...
	select *,sum(pia_cantsolicitada ) as total from mwktotval2,mwkpres ;
		...
		group by pre_especialidad,PRE_modoatencion  , VAL_codservvale, VAL_circuitoorigen &mgroup ;
...
select ESP_codesp, ESP_descripcion, VAL_codservvale,VAL_fechasolicitud,PRE_modoatencion  , ;
	...
	from mwktotval1 ;
	group by ESP_descripcion,PRE_modoatencion    &mgroup ;
	order by ESP_descripcion,PRE_modoatencion    &mgroup into cursor mwktotval
