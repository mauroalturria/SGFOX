...
if reccount('mwkVerEvol')= 0
	...
	ELSE
		mret = sqlexec(mcon1, "insert into TabGuaEvolNurse (EGN_evolNurse,EGN_codCIENanda,"+;
			" EGN_fechaH ," + ;
			" EGN_parAdmF , EGN_parAlerg , EGN_parFreCard , EGN_parFreResp , "+;
			" EGN_parGluc , EGN_parOtros , EGN_parPeso , EGN_parSatur , EGN_parTemAxl , "+;
			" EGN_parTemBuc , EGN_parTemRct , EGN_parTensDia , EGN_parTensSis,"+;
			" EGN_parAlergQue,"+;
			" EGN_codCIENanda "+;
			" ) values "+;
			" (?mevolNurse,?mcodCIENanda,?mfechaHora,?mparAdmF , ?mparAlerg , "+;
			" ?mparFreCard, ?mparFreResp , ?mparGluc ,"+;
			" ?mparOtros  , ?mparPeso , ?mparSatur , ?mparTemAxl , ?mparTemBuc , "+;
			" ?mparTemRct ,"+;
...
*****preguntar si va esto, o solo en el insert
		mret = sqlexec(mcon1, "update TabguaevolNurse set EGN_evolNurse =?mevolNurse, EGN_parAdmF = ?mparAdmF "+;
			", EGN_parAlerg = ?mparAlerg , EGN_parFreCard=?mparFreCard , EGN_parFreResp =?mparFreResp, "+;
			" EGN_parGluc =?mparGluc, EGN_parOtros=?mparOtros  , EGN_parPeso = ?mparPeso , EGN_parSatur=?mparSatur "+;
			", EGN_parTemAxl =?mparTemAxl,EGN_parTemBuc =?mparTemBuc, EGN_parTemRct=?mparTemRct ,"+;
			" EGN_parTensDia =?mparTensDia, EGN_parTensSis = ?mparTensSis,EGN_parAlergQue=?mparAlergQue "+;
			",EGN_codCIENanda = ?mcodCIENanda where id = ?mid ")
	endif
endif
if mret < 0
	=aerr(eros)
