...
Private mUse , mFec ,nIdFicha, mFechaPasivo

llRespuesta = .T.									&& Respuesta si el proceso se corrio satisfactoriamente
...
If lnReturn > 0
   ...
   If Reccount('mwkEpidemioCOV19Cont') <> 0
      ...
      *-- Carmen sugirio borrado logico de los datos para no perderlos 05/04/2021
      lnReturn = SQLExec( mcon1, "Update ZabCov19Contacto SET ZabCov19Contacto.CON_FecPasiva = ?mFechaPasivo WHERE ZabCov19Contacto.CON_Idplanilla = ?mvID ", "mwkEdidat" )
     
      If lnReturn < 0
         =Aerror(merror)
         Messagebox("EN ACTUALIZACION DE CONTACTOS para COV19 (Update) " + Chr(10) + Alltrim(merror(3)),16,"ERROR")
         Do log_errores With Error(), Message(), Message(1), Program(), Lineno()
         llRespuesta = .F.							&& Si paso por aca fallo el Update 
      Endif

      *-- Controlo como  viene la grabacion de datos, si fallo el Update o no
      If llRespuesta = .T.
         ...
         * Go Top
         ...
         Scan All
            ...
            TEXT TO cmdSQL text NOSHOW PRETEXT 7
		         INSERT INTO ZabCov19Contacto
		            ( CON_Idplanilla,
		            CON_ApeNom,
		            CON_documento,
		            CON_Telefono,
            ...
            lnReturn = SQLExec(mcon1, cmdSQL,"mwkContactosTemp" )

            If lnReturn < 0
               =Aerror(merror)
               Messagebox("EN INCORPORACION DE CONTACTOS para COV19 (Update) " + Chr(10) + Alltrim(merror(3)),16,"ERROR")
               Do log_errores With Error(), Message(), Message(1), Program(), Lineno()
               mRespuesta = .F.
            Endif
...
*-- Valor de si se efectuo la operación o no
Return mRespuesta
