...
mret = sqlexec(mcon1, "select PAC_habitacion , PAC_cama, PAC_nombrepaciente " + ;
	...
	", Tabnutpachist.*,sec_descripsec,PAC_fechaalta, pac_categoria " + ;
	", entidexclu.fecpasiva, sec_codsector "+;
	", PAC_codhci "+;
	" from Tabnutpachist inner join pacientes on Tabnutpachist.TNP_codadmision = pacientes .PAC_codadmision " + ;
...
if nserv>0
	mret = sqlexec(mcon1, "select PAC_habitacion , PAC_cama, PAC_nombrepaciente " + ;
		...
		", TNP_codfactu,TNP_factura,TNP_dieta, entidexclu.fecpasiva, sec_codsector ,ent_descrient "+;
		", PAC_codhci "+;
	...
	select mwknutdieta1.*, mwknutdieta2.tnp_codfact as codfact, mwknutdieta2.tnp_observaciones as indicacion,ENT_descrient ;
		,nvl(hora0,2400) as hora0, 2400-2400 as horaserv;
		,nvl(mwknutdieta2.tnd_observa ,space(50)) as comentario;
		from mwknutdieta1 left join mwknutdieta2 on mwknutdieta1.PAC_codadmision = mwknutdieta2.PAC_codadmision ;
	...
	if reccount('mwknutdietaSD')>0 and nserv <5
		...
		select PAC_habitacion, PAC_cama,PAC_nombrepaciente ,PAC_fechaadmision, PAC_descripdiagn ;
			, PAC_horaadmision , cob_codentidad ,PAC_codadmision,PAC_edad,PAC_fecnacimiento  ;
			, TNP_Fecha ,TNP_CodServ, tnp_codfact, codfact,1 as TNP_factura,tnp_observaciones;
			, TNP_Usuario,space(50) as tnd_observa, 1 as TND_idPaciente,0 as TND_codPrest,0 as TND_NroVale,datetime() as TND_FHoraCarga;
			,0 as TND_Cantidad,sec_descripsec, fecpasiva, indicacion,codfact,pac_fechaalta,ENT_descrient ;
	...
	select mwknutdieta1.*,tnp_observaciones as indicacion,tnp_codfact as codfact, ENT_descrient;
		,2400 as hora0, 0000 as horaserv;
		,nvl(tnd_observa,space(50)) as comentario ;
		from mwknutdieta1 ;
		into cursor mwknutdieta
endif
...
if ldiant = 1
*** Nutricion anterior
	mret = sqlexec(mcon1, "select TNP_codadmision,TNP_Observaciones,tnp_modi  from Tabnutpachist " + ;
	...
	select PAC_habitacion +'-'+PAC_cama as habitac,PAC_nombrepaciente  ;
		...
		, mwknutant1.tnp_observaciones as adnutant, sec_codsector,tnp_modi,PAC_fecnacimiento ;
		,PAC_codhci,dtoc(pac_fechaadmision)+" " +ttoc(pac_horaadmision, 2) as pacfechaadmision ;
		from mwknutdieta ;
		...
		into cursor mwkdietaaux

else
	if reccount('mwknutdieta')>0

		select PAC_habitacion +'-'+PAC_cama as habitac,PAC_nombrepaciente  ;
			, proper(PAC_descripdiagn) as PAC_descripdiagn ;
			, PAC_fechaadmision, PAC_horaadmision , cob_codentidad as pin_codentidad,PAC_codadmision ;
			,prg_edad(PAC_fecnacimiento,mfecha,"AM") as anios ;
			, TNP_Fecha ,TNP_CodServ, tnp_codfact, codfact_a as TNP_codfactu,TNP_factura, tnp_observaciones;
			, TNP_Usuario,tnd_observa, TND_idPaciente,TND_codPrest,TND_NroVale,TND_FHoraCarga,pre_descriprest,TND_Cantidad ;
			,sec_descripsec, iif(isnull(mwknutdieta.fecpasiva),"  ","PE") as pe, indicacion,codfact_b as codfact,pac_fechaalta ;
			,iif(hora0>horaserv,hora0,horaserv) as TND_Hora,max(comentario) as comentario, pac_categoria;
		...
		select PAC_habitacion +'-'+PAC_cama as habitac,PAC_nombrepaciente  ;
			...
			, PAC_fechaadmision, PAC_horaadmision , cob_codentidad as pin_codentidad,PAC_codadmision ;
			,prg_edad(PAC_fecnacimiento,mfecha,"AM") as anios ;
			, TNP_Fecha ,TNP_CodServ, tnp_codfact, 0 as  TNP_codfactu,0 as TNP_factura, tnp_observaciones;
			, TNP_Usuario,space(50) as tnd_observa, 1 as TND_idPaciente,0 as TND_codPrest, 0 as TND_NroVale,datetime() as TND_FHoraCarga,"" as pre_descriprest,0 as TND_Cantidad ;
			,sec_descripsec, iif(isnull(mwknutdieta.fecpasiva),"  ","PE") as pe, indicacion,codfact,pac_fechaalta ;
			,iif(hora0>horaserv,hora0,horaserv) as TND_Hora,max(comentario) as comentario, pac_categoria;
			,"" as adnutant, sec_codsector ,tnp_modi,PAC_fecnacimiento ;
			,PAC_codhci,dtoc(pac_fechaadmision)+" " +ttoc(pac_horaadmision, 2) as pacfechaadmision ;
...
select * from mwkdietaaux into cursor mwkdieta 
