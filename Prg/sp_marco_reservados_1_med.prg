...
mret=SQLExec(mcon1,"SELECT  B.DIASEM,B.CODMED,B.CODESP,B.INTERNADO,B.GUARDIA,b.HORArDESDE, b.HORADESDE,"+;
	" b.HORArHASTA,b.HORAHASTA,B.TIPOTURNO,a.HORADESDE as hdesfran,a.HORAHASTA as hhasfran,TT.GRUPO "+;
	" FROM medpresta AS a,TABRESERVADO AS B,tabtipoturno as tt "+;
	" WHERE &mccpoamb centromedico = ?mxcentromedico and A.CODMED=B.CODMED AND B.Diasem=?mddiasem AND generaagen >0 "+;
	" AND a.Diasem=?mddiasem AND  a.horadesde is not null "+;
	" and b.fecvigenh >= ?mdmasx and a.fecvigenh >= ?mdmasx and b.codmed = ?mncodmed " + ;
	" and b.fecvigenh > b.fecvigend and B.TIPOTURNO = tt.TIPOTURNO  "+;
	" ORDER BY B.codmed","MWKResGuardiaIntera")
Select DIASEM,CODMED,CODESP,INTERNADO,GUARDIA,Nvl(HORArDESDE,HORADESDE) As HORADESDE,;
	nvl(HORArHASTA,HORAHASTA) As HORAHASTA,TIPOTURNO,hdesfran,hhasfran,GRUPO From MWKResGuardiaIntera;
	...
	Scan
		...
		Else
			If mihorad = Ctot('00:00:00')
				...
				mthorah     = MWKResGuardiaInter.HORAHASTA
			Else
				mthorad     = IIF(HOUR(mihorad)<HOUR(MWKResGuardiaInter.HORADESDE),MWKResGuardiaInter.HORADESDE,mihorad)
				mthorah     = IIF(HOUR(mihorah)>HOUR(MWKResGuardiaInter.HORAHASTA),MWKResGuardiaInter.HORAHASTA,mihorah)
			Endif
