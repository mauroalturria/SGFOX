	...
	PROCEDURE EnumKeys
		...
		DO WHILE .T.
			...
			CASE m.nErrCode = ERROR_EOF
				Exit
			CASE m.nErrCode = ERROR_MORE_DATA	
				&& continuo			
		...
		* 182 "not enough memory"
		...
		DO WHILE .T.
			...
			* Return drivers or data sources
			IF m.lDataSources
				nRetVal = SQLDataSources(m.nODBCEnv,SQL_FETCH_NEXT,@dsn,999,@mdsn,@dsndesc,255,@mdesc)
			ELSE
				nRetVal = SQLDrivers(m.nODBCEnv,SQL_FETCH_NEXT,@dsn,999,@mdsn,@dsndesc,999,@mdesc)
			ENDIF
