...
*
Lparameters mdesde, mhasta, midmedico, mbusAmb, mBusTur
...
*mret = sqlexec(mcon1,"select * from prestacions","mwkpresta")

mret = sqlexec(mcon1,"select fechahoraing as fhturno,fechahoraate as fhatiende,"+;
	"REG_nombrepac as paciente,PRE_descriprest as prestacion,ENT_descrient as entidad,"+;
	"REG_fecnacimiento as fnac,TAM_mensaje as mensaje, TabAmbulatorio.protocolo,"+;
	" TabAmbulatorio.id ,TabAmbulatorio.codmed,TabAmbulatorio.codent,TabAmbulatorio.codestado, "+;
	"Pre_Especialidad, Pre_CodServicio, fechaconfirma as fechahoraing, " + ;
...
mret = sqlexec(mcon1, 'select turnos.id, turnos.fechatur, turnos.horatur, turnos.codesp,' + ;
	...
	'Pre_Especialidad, fechaconfirma as fechahoraing,' + ;
	'Prestadores.NOMBRE AS NOMBRE ' + ;
...
Select horatur,reg_nombrepac,codprest,codent,fechanac,reg_nroregistrac,sala,codmed,nrovale,prestacion, ;
	codserv, Pre_Especialidad as CodEsp, fechahoraing,nombre ;
    from mwkphorario1;
	union ;
select horatur,reg_nombrepac,codprest,codent,fechanac,0 as reg_nroregistrac,sala,codmed,nrovale,prestacion, ;
	codserv, Pre_Especialidad, fechahoraing,nombre ;
	from mwkphorario2;
	into cursor mwkphorarios

*!* Select horatur as fhturno,iif(ttod(fechahoraate)=ctod("01/01/1900"),{//},fechahoraate) as fhatiende,;

Select horatur as fhturno,iif(ttod(mwkphorarios.fechahoraing)=ctod("01/01/1900"),{//},mwkphorarios.fechahoraing) as fhatiende,;
	reg_nombrepac as paciente,mwkphorarios.prestacion,;
	mwkentidad.ENT_descrient as entidad,fechanac as fnac ,TAM_mensaje as mensaje,;
	mwkAmbula.protocolo as protocolo, mwkAmbula.id , sala,mwkphorarios.codmed,;
	mwkphorarios.codent,mwkAmbula.codent as codent1,codestado, CodEsp, CodServ, mwkphorarios.fechahoraing as fechahoraing,nombre ;
	from mwkphorarios;
	left join mwkentidad on ENT_codent = mwkphorarios.codent ;
	left join mwkAmbula on mwkAmbula.codprest = mwkphorarios.codprest and ;
	mwkAmbula.nrovale = mwkphorarios.nrovale;
...
If reccount('mwkdemanda')>0
	...
	Select fhturno,fhatiende,paciente,prestacion,entidad,fnac,mensaje,;
		protocolo,id,sala,codmed,iif(isnull(codent1),codent,codent1) as codent ;
		,nvl(codestado,20) as codestado, CodEsp, CodServ, fechahoraing,nombre;
		from mwkambu1;
	union;
	select fhturno,fhatiende,paciente,prestacion,entidad,fnac,mensaje,;
		protocolo,id,space(20) as sala,codmed,codent,codestado, Pre_Especialidad, Pre_CodServicio, fechahoraing,nombre;
		from mwkdemanda;
		into cursor mwkambu
