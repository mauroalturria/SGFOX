...
CASE type('mcodadmi') = "C"
	mret = sqlexec(mcon1, "select PAC_nombrepaciente,PAC_codadmision, PAC_habitacion, PAC_cama, PAC_sectorinternac," + ;
		"PAC_domicilio, PAC_codhci, PAC_codhce, ENT_codent, ENT_descrient,ENT_nroprestadorexterno,COB_codcontrato, TPV_Estado  " + ;
		" ,registracio.*, COB_CondicImpositiva,PAC_Tipopac,  PAC_tipopaciente "+;
		...
		"where PAC_codadmision = ?mcodadmi and pac_codhci= registracio.REG_nroregistrac "+ ;
		"group by PAC_codadmision " , "mwkencontrado")
	if mret < 0
