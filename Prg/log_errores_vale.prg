...
*
Lparameters lnError, lcMsg, lcMsg1, lcProg, lnLinNro
...
lcError = "Hora: " 	   + Time()		 + Chr(13) + Chr(10)+ ;
	...
	"ERROR:    " + Str(lnError)	 + Chr(13) + Chr(10)
...
=Fwrite(gnErrFile, "*-----------------------------------------------------------" + Chr(13) + Chr(10))
for yi = 1 to 27
	lcError = transform(dat_vale(yi))+" tipo: "+vartype(dat_vale(yi))+ Chr(10)
	=Fwrite(gnErrFile,	lcError)
next
=Fclose(gnErrFile)
...
*!*					+ "|" + "Msg:" + lcMsg+ "|" + "Msg1:" + lcMsg1+ "|" + "Err:" + Str(lnError)
			...
			Do sp_insert_tabCtrlErr With lcError ,myip, mUser, mwkexe.nomexe,;
