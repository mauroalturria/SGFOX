...
Endif
If !Used('mwkservres')
	Do sp_busco_tabintserv With 3,,'mwkservires'
Endif
Select ESP_descripcion ,ih_admision From mwkservires Group By IH_admision  Into Cursor mwkespresp
...
" where hab_sectores = ?mcodsec and pac_centromedico = ?mxcentromedico "+mwcm, "mwkcama0b")
...
*!*	Endif
IF INLIST( mcodsec ,"EME",'CEG','IDE','IEP') Or mcodsec$sectoresped
	...
	Select  mwkcama0.*,THC_Cor,	Iif(INLIST(PAC_sectorinternac,"EME",'CEG','IDE','IEP'),"Ingreso:"+Dtoc(PAC_fechaadmision)+" - ",'')+Alltrim(pac_descripdiagn) As pac_diagn, ;
	iif(Isnull(IdMotivo) And !Isnull(pac_nombrepaciente),0,Iif(atendido,0,Iif(!Isnull(pac_nombrepaciente),1,0))) As Id_Motivo ,rc_estado,;
	COV_HisopaVigilan,RC_hisopadoFecha ,mwkespresp.ESP_descripcion  As servresp ;
	from mwkcama0;
	LEFT Join mwkespresp On IH_admision = hab_codpaciente;
...
Else
	...
	iif(Inlist(PAC_sectorinternac,"EME",'CEG','IDE','IEP'),"Ingreso:"+Dtoc(PAC_fechaadmision)+" - ",'')+Alltrim(pac_descripdiagn) As pac_diagn, ;
	0 As Id_Motivo,rc_estado,COV_HisopaVigilan,RC_hisopadoFecha,mwkespresp.ESP_descripcion  As servresp   ;
	from mwkcama0;
	LEFT Join mwkespresp On IH_admision = hab_codpaciente;
	Left Join mwkepicov  On  admisionficha = hab_codpaciente ;
	left Join mwkCorcama On hab_codhabitacion = THC_Hab And hab_codcama = THC_Cama ;
...
If mhabitsala = 'H'  And Reccount("mwkcama0b")=0
	...
	,Iif(hab_codcama = '01',Iif(Isnull(horaaud),Space(20),Ttoc(horaaud)),'        ') As hora1 ;
		,Iif(hab_codcama = '01',"Servicio resp.:"+Nvl(servresp ,SPACE(40)) ,SPACE(40)) As servresp_1 ;
,Iif(hab_codcama = '01',Nvl(rc_estado,0) ,0) As rc_estado_1 ;
	...
	,Iif(hab_codcama = '02',"Servicio resp.:"+Nvl(servresp ,SPACE(40)) ,SPACE(40)) As servresp_2 ;
	...
	,Max(servresp_1) As servresp_1,Max(rc_estado_1) As rc_estado_1,Max(COV_hv_1) As COV_hv_1,  Max(RC_hf1) As RC_hf1,  ;
	...
	,Max(servresp_2) As servresp_2,Max(rc_estado_2) As rc_estado_2,Max(COV_hv_2) As COV_hv_2,Max(RC_hf2) As RC_hf2,   ;
...
Else
	If  Reccount("mwkcama0b") > 0
		Selec hab_codhabitacion;
		,hab_codcama  As cam1 ;
		,Padr(Iif(Empty(Nvl(pac_categoria,'')) And Empty(Nvl(codaisla,' ')),' ',;
		Iif(Empty(Nvl(codaisla,'')),pac_categoria,codaisla)),20) As pac1 ;
		...
		,"Servicio resp.:"+Nvl(servresp ,SPACE(40))  As servresp_1 ,Nvl(rc_estado,0) As rc_estado_1 ,Nvl(COV_HisopaVigilan,0) As COV_hv_1, 0 As COV_hv_2;
		,SPACE(40) As servresp_2,0 As rc_estado_2 ,Nvl(RC_hisopadoFecha,Ctod("01/01/1900")) As RC_hf1,Ctod("01/01/1900") As RC_hf2 ;
		from mwkcama1 Into Cursor mwkc2

		Selec hab_codhabitacion As hab1, cam1, pac1, ;
		ent1, nom1, sex1,  eda1, dia1,codp1, ;
		hab_codhabitacion As hab2, cam2, pac2,'' As tipoaisla1,'' As codaisla1,'' As hora1 ;
		,ent2, nom2, sex2, eda2, dia2,codp2,'' As tipoaisla2,'' As codaisla2,''As hora2 ;
		, cama_esp_1, cama_esp_2,Id_Motivo,servresp_1,rc_estado_1,COV_hv_1,COV_hv_2 ,servresp_2,rc_estado_2,RC_hf1,RC_hf2   ;
	...
	Else
		...
		a.pac_edad As eda1, a.pac_diagn As dia1,a.hab_codpaciente As codp1,'' As tipoaisla1,'' As codaisla1,'' As hora1,;
		"Servicio resp.:"+Nvl(a.servresp ,SPACE(40))  As servresp_1 ,Nvl(a.rc_estado,0) As rc_estado_1 ,;
		...
		Iif(Empty(Nvl(b.codaisla,'')),b.pac_categoria,b.codaisla)),20) As pac2,'' As tipoaisla2,'' As codaisla2,'' As hora2;
		,"Servicio resp.:"+Nvl(b.servresp ,SPACE(40))  As servresp_2,Nvl(a.rc_estado,0) As rc_estado_2 ,;
...
"where c.POA_vigente = 1 and NVL(d.sec_circaltas,0) = 1 "+mwcm+;
"group by c.poa_sector, c.poa_cama, c.poa_habitacion) as k on a.id = k._id " +;
"where a.poa_sector = ?mcodsec and a.poa_vigente = 1 and a.POA_estado in (4,11,12) ", "mwkAltapac")
...
"padr(iif(at('A x',A.pac1)>0,A.pac1, iif(A.pac1 = 'A', 'AISL s/AC', iif(A.pac1 = 'I', 'IND ', iif(A.codp1 = 'BLOQUEO'," + ;
" 'BLOQUEO', iif(A.codp1 = 'RESERV', 'RESERV', '    '))))),20) as est1, "+ ;
...
"iif(isnull(A.cam2), '  ', A.cam2) as cam2, PADR(iif( at('A x',A.pac2)>0,A.pac2,iif( A.pac2 = 'A', 'AISL s/AC', iif(A.pac2 = 'I',"+;
" 'IND ', iif(A.codp2 = 'BLOQUEO'," + ;
" 'BLOQUEO', iif(A.codp2 = 'RESERV', 'RESERV', '    '))))),20) as est2, " + ;
...
"A.tipoaisla1,A.codaisla1,A.hora1,a.servresp_1,a.rc_estado_1 ,A.hora2,A.tipoaisla2,A.codaisla2,a.servresp_2,a.rc_estado_2,a.RC_hf1,a.RC_hf2,a.COV_hv_1,a.COV_hv_2, " +;
