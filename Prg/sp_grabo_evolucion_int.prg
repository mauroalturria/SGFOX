...
Parameters lgrabobien,mnroreg, mprot, mmedico,mopage2, mopage3,mopage4,mopage5,mopage6,mopage7,musua,morigen,lrevision
Dimension admfar(11),pract(13)
Dime porcMC(5)
If Vartype(lrevision)<>"N"
	lrevision =0
Endif
*
...
mlsolocie10 = mopage2.Parent.Parent.lsolocie10
...
mlcambioalim  =  mopage2.Parent.Parent.lcambioalim
...
mintercons = ''
With mopage2.PgEvol.pgEvolIC.Pgintercons1.pgInterSel &&(**)
	mesp = ''
	If .pgSelInter.optopcion.Value >0
		If .pgSelInter.optopcion.Value <3
			mesp =  Alltrim(.pgSelInter.pageic.pgant.txtpresta.Value) +Chr(10)
		Else
			With .pgSelInter.pageic.pgExt
	...
	mintercons = mesp + Alltrim(.pgevolucion.txteditEvol.Value )+ Iif(!Empty(mievolm),Chr(10)+"Evolución:"+ Chr(10)+mievolm + Chr(10),'')
Endwith
...
****  actualiza SNG
		...
		If mSNG = 2 And Reccount('mwkSNG')>0   &&&& da por finalizada
			...
			Endif
		Else &&& actualiza o inserta
			If (sisng Or sisngn) And (mlcambiosng Or mlcambiosngn)
				mret = SQLExec(mcon1, "SELECT id FROM TabIntAVN " + ;
...
****  actualiza SVES
		...
		Else &&& actualiza o inserta
			...
			Endif
		Endif
****  actualiza alimentacion
		Select mwkAlim
		mAlim = mALalimenta
		mAlimacc = mwkNuttipo.Id
		If mAlim = 2 And Reccount('mwkAlim')>0   &&&& da por finalizada
			mret = SQLExec(mcon1, "update TabIntAVN set AVN_fechafin= ?mfechadia "+;
				" ,AVN_fechaH = ?mfechahora  ,AVN_usuario = ?midusuario   where AVN_tipo= 15 and AVN_idevol = ?midevol and AVN_fechafin > ?mfechadia  ")
			If mret < 0
				Do log_errores With Error(), Message(), Message(1), Program(), Lineno()
			Endif
		Else &&& actualiza o inserta
			If  mAlim = 1 And mlcambioAlim
				mret = SQLExec(mcon1, "SELECT id FROM TabIntAVN " + ;
					" where  AVN_idevol = ?midevol and AVN_tipo = 15  and AVN_fechafin = ?mfecfin ", "mwkVerAlim")

				If mret < 0
					Do log_errores With Error(), Message(), Message(1), Program(), Lineno()
				Endif

				miAlim = mwkVerAlim.Id
				If miAlim = 0 &&&inserto
					mAlimfini =  Ttod(mwkfecserv.fechahora)
					mret = SQLExec(mcon1, "insert into TabIntAVN " + ;
						" ( AVN_complica,AVN_fechafin,AVN_fechaH,AVN_fechaini,AVN_idevol,AVN_modo,AVN_motivo,AVN_tipo,AVN_usuario)"+;
						" values "+;
						" (0,?mfecfin, ?mfechahora,?mAlimfini , ?midevol,?mAlimacc , 0, 15,?midusuario  )" )
				Endif
				If mret < 0
					Do log_errores With Error(), Message(), Message(1), Program(), Lineno()
				Endif
...
If musua = 0		&&&& datos medicos
	If mlcambioepicris Or mlfinepicris
		If mwktaltasint.tipoest<9
			...
			Else && modo revision
				If  mlcambioepicris
					mret = SQLExec(mcon1, "update TabIntHCE set   IH_codestado = ?miestado "+;
						",IH_codmedcie = ?mmedico,IH_codcie = ?mcodcie10,IH_resumen = ?mIH_resumen  "+;
						" where Id = ?midevol ")
				Endif
			Endif
...
**codcie10 ,descrip,idcie10,IE_codcie ,IE_codmed ,IE_patologia,IE_fechaHora,IE_fechaHBaja, IE_idevol ,IE_tipo
	...
	Else
		If mlcambioanam Or mlcambioanamllave
			mret = SQLExec(mcon1, "update TabIntHCE set IH_motIngreso = ?mIH_motIngreso , IH_procedencia = ?mIH_procedencia "+;
				" ,IH_reingre = ?mIH_reingre where Id = ?midevol ")
		Else
			If mlsolocie10
				mret = SQLExec(mcon1, "update TabIntHCE set IH_motIngreso = ?mIH_motIngreso where Id = ?midevol ")
			Endif
		Endif
	...
	If mlcambioepicris Or mlfinepicris
		...
		If miestado <=9
			mcampoest = Iif(lrevision = 0,"IH_horaCierre = ?mfechahora , ","")+" IH_codestado = ?miestado"+Iif(!mlcambioepicris ,'',",IH_codmedcie = ?mmedico ")
		Endif
		mcodcie10 = mwkcie9.Id
		mIH_resumen = mopage6.txteditresumen.Value
