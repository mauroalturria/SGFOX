...
if not used('mwkprestaAC')
    mret = sqlexec(mcon1,"select pre_descriprest, pre_codprest , pre_codservicio, " + ;
        "pre_especialidad,pre_tipomuestra from prestacions " + ;
...
mret = sqlexec(mcon1, "select PAC_nombrepaciente, PAC_edad " + ;
    ...
    ", autprevias.apv_diagnostico , autprevias.apv_resprev, afi_nroafiliado ,pre_codservicio,pre_tipomuestra "+;
    ", apv_descripsolic, apv_descripautori, apv_posologiasol, pac_tipopac,APV_NroVale,pre_descriprest  "+;
	", APVP_cadera24,APVP_justifPorta, APVP_planATB,APVP_tipoaisla,PRE_Lateralidad "+;
    " from autprevias " + ;
	" INNER JOIN TabAutObs ON AutPrevias.Id = TabAutObs.IdAutPrevias  " + ; 
    " left join prestacions on pre_codprest = apv_codprestsolic "+;
...
select pac_nombrepaciente, pac_edad , sec_codsector, sec_descripsec, pac_habitacion, pac_cama ;
    ...
    , apv_subestadopend,pre_descriprest  ;
    , pre_descriprest as pre_descriprest1, estse.descrip , estse.subestado ;
    , autid, mwkentexc_int.fecpasiva, pre_codservicio ;
    , apv_diagnostico , apv_resprev, afi_nroafiliado ;
    , apv_descripsolic, apv_descripautori, apv_posologiasol, pac_tipopac ;
    ,observa,FechaObs, mwkpacint00.estado, usuario, IdObs,estaob.descrip as EstadoObs ,APV_NroVale,pre_tipomuestra;
    , APVP_cadera24,APVP_justifPorta, APVP_planATB,APVP_tipoaisla ;
    , NVL(PRE_Lateralidad,0) as PRE_Lateralidad ,prg_busco_dato_estsolic(apv_idautprevias,APV_CodPrestSolic ,1) as apv_codlado,ENT_nroprestadorexterno ;
    from mwkpacint00;
...
select  PAC_nombrepaciente;
	...
	,APV_HoraSolicitud, PRE_codservicio, PAC_descripdiagn ,APV_ResPrev, AFI_nroafiliado ;
	,nvl(APV_DescripSolic,space(30)) as APV_DescripSolic, nvl(APV_DescripAutori,space(30)) as  APV_DescripAutori;
	, nvl(APV_PosologiaSol,space(30)) as APV_PosologiaSol, pac_tipopac,observa,APV_NroVale,pre_tipomuestra ;
	, NVL(APVP_cadera24,0) as APVP_cadera24,NVL(APVP_justifPorta,0) as APVP_justifPorta, ;
	NVL(APVP_planATB,0) as APVP_planATB,NVL(APVP_tipoaisla ,0) as APVP_tipoaisla,PRE_Lateralidad,apv_codlado,ENT_nroprestadorexterno  ;
	from mwkpacint0   ;
	order by APV_FechaSolicitud desc,PAC_nombrepaciente,FechaObs DESC into cursor &mNomCur
