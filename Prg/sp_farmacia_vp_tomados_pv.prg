...
*!*			mret   = SQLExec(mcon1,"update TabFarmVPCabe set TVC_FinPre = ?mfechor, TVC_Estado = 2, TVC_EstadoKar = 1, TVC_EstadoExt = 1 where ID = ?mbusca")
		mret   = SQLExec(mcon1,"update TabFarmVPCabe set " +;
			"TVC_FinPre = ?mfechor, " +;
			"TVC_EstadoKar = 1, "+;
			"TVC_EstadoExt = 1 " +;
			"where ID = ?mbusca")
...
***mret   = SQLExec(mcon1,"update TabFarmVPCabe set TVC_FinPre = ?mfechor, TVC_Estado = 2, TVC_EstadoExt = 1, TVC_EstadoKar = 1 where ID = ?mbusca")
			mret   = SQLExec(mcon1,"update TabFarmVPCabe set TVC_EstadoExt = 1, TVC_EstadoKar = 1 where ID = ?mbusca")
		Else
	...
	Case mform.pPantalla = "K"  &&Kardex
		...
		If lCierraTodo
***mret   = SQLExec(mcon1,"update TabFarmVPCabe set TVC_FinPre = ?mfechor, TVC_Estado = 2, TVC_EstadoKar = 1, TVC_EstadoExt = 1 where ID = ?mbusca")
			mret   = SQLExec(mcon1,"update TabFarmVPCabe set TVC_EstadoKar = 1, TVC_EstadoExt = 1 where ID = ?mbusca")
		Else
...
*!*			"(a.tvc_EstadoExt is not null and a.tvc_EstadoKar is not null) ","mwkconpre1")
	...
	mret = SQLExec(mcon1,"select a.id, a.TVC_IniPre,a.TVC_FinPre,a.TVC_Estado, a.TVC_EstadoExt, a.TVC_EstadoKar," +;
		...
		"d.VAL_fechaconforme ,d.VAL_horaconforme, d.VAL_OperadorConforme, " +;
		"d.VAL_fechacargasoli, d.VAL_horacargasolic, d.VAL_Observaciones,d.VAL_medicosolicit, d.VAL_Lugar_Origen, " +;
		"nvl(a.TVC_ImpreExt,0) as TVC_ImpreExt, NVL(a.TVC_impreKar,0) as TVC_impreKar,TVE_tipo, TVE_Armario ,NVL(b.TVE_Frio,0) as TVE_Frio, NVL(b.TVE_ParaAlta,'N') as TVE_ParaAlta " +;
		"from TabFarmVPCabe as a " +;
		"left join TabFarmVPDetEnt as b on a.id = b.TVE_IdPre " +;
		"left join Insumos as c on b.TVE_insumocodigo = c.ins_codinsumo " +;
		"left join valesasist as D on b.TVE_Vale = d.val_codvaleasist " +;
...
* Marcelo Torres, 19/03/2025. Se agregó a la consulta el campo TVE_tipo.
* Para que tome la ubicación del insumo desde la propia tabla TabFarmVpDetEnt
* Asi evitamos consultar multiples veces al servicio.
...
Case mtipo = 'I'   &&Traemos una preparación
...
**SET STEP ON
	...
	mret = SQLExec(mcon1,"select a.id, a.TVC_IniPre,a.TVC_FinPre,a.TVC_Estado, a.TVC_EstadoExt, a.TVC_EstadoKar," +;
		...
		"d.VAL_fechaconforme ,d.VAL_horaconforme, d.VAL_OperadorConforme, " +;
		"d.VAL_fechacargasoli, d.VAL_horacargasolic, d.VAL_Observaciones,d.VAL_medicosolicit, d.VAL_Lugar_Origen, " +;
		"nvl(a.TVC_ImpreExt,0) as TVC_ImpreExt, NVL(a.TVC_impreKar,0) as TVC_impreKar,TVE_tipo, TVE_Armario,H.PAC_nombrepaciente, PAC_edad,NVL(b.TVE_Frio,0) as TVE_Frio, NVL(b.TVE_ParaAlta,'N') as TVE_ParaAlta, I.REG_numdocumento " +;
		"from TabFarmVPCabe as a " +;
		"left join TabFarmVPDetEnt as b on a.id = b.TVE_IdPre " +;
		"left join Insumos as c on b.TVE_insumocodigo = c.ins_codinsumo " +;
		"left join valesasist as D on b.TVE_Vale = d.val_codvaleasist " +;
		...
		"left join TabEstados as F on f.SubEstado = e.TVO_SubEstado and f.Propietario = 32 " +;
		"left join prestadores as G on g.id = e.TVO_codmed " +;
		"left join pacientes as H on D.Val_codadmision = H.PAC_codadmision " +;
		"left join registracio as I on H.Pac_codhci = I.Reg_nroregistrac " +;
		"where a.id = ?midpre","mwkconpre1")
...
**mform, mtipo, midpre
	...
	If mProceso > 0
		...
		cTextoErr = " ACTUALIZACION "
		If mret < 0
			mltabla =  cTextoErr+"ESTADO DE PROCESO - CABECERA"
			Do Log_errores With Error(), Message(), Message(1), Program(), Lineno()
			Messagebox("EN LA TABLA "+mltabla +Chr(10)+"AVISE A SISTEMAS",16,"ERROR")
			mretorno = .F.
		Else
	...
	Else
		mret   = SQLExec(mcon1,"select TVC_Proceso from TabFarmVPCabe where ID = ?mbusca","mwkProceso")
		cTextoErr = " CONSULTA "
		If mret < 0
			mltabla =  cTextoErr+"ESTADO DE PROCESO - CABECERA"
			Do Log_errores With Error(), Message(), Message(1), Program(), Lineno()
		...
		Else
			...
			mretorno = Nvl(mwkProceso.TVC_Proceso,0)

			Use In Select("mwkProceso")
...
Case mtipo = "T"  &&Terminamos preparacion.
	...
	If mret < 0
		mltabla = "CIERRE DE PREPARACIÓN - CABECERA"
		Do Log_errores With Error(), Message(), Message(1), Program(), Lineno()
		Messagebox("EN LA TABLA "+mltabla +Chr(10)+"AVISE A SISTEMAS",16,"ERROR")
		Return .F.
	Endif
