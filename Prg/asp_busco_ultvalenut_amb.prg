...
fechalimite = mfechabaja - 3600*24 && LE RESTE UN DIA
...
mret = SQLExec(mcon1, "Select Guardia.*, GuardiaVale.NroVale " + ;
	...
	"and tabtipoaltas.TipoEst > 0 ", "mwkGuardia1")
...
If mwkGuardiaf.fechavale = Ctot('')
	mfechaval = mfechabaja 
Else 
	mfechaval = mwkGuardiaf.fechavale
Endif 	
...
mret = SQLExec(mcon1, "select tnd_nroVale from TabNutDetAmb "+;
	" where TND_FHoraCarga >= ?mfechaval ", "mwkvales")
