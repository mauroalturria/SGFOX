...
&& and TNDP_fecpasiva = ?mfecpas  por ahora se lo saco
	...
	** no compatibles
	...
	select PAC_codadmision,TNC_idIngr  from mwkauxiprest left join mwknutdesa02 on TND_codPrest = TNC_codPrest;
		group By PAC_codadmision,TNC_idIngr having TNC_idIngr>0 ;
	...
	select * From mwknutdesa01 Where alltrim(TNP_codadmision )+transf(id,"999999");
		not in (select alltrim(PAC_codadmision )+transf(TNC_idIngr,"999999") from mwkingNoComp) ;
		into cursor mwknutdesa01a
	
		
	Select * From mwknutdesa01a ;
		union Select * From mwknutdesa11 ;
			union Select * From mwknutdesa1 Where !Isnul(Id);
				into Cursor mwknutdesau

	select * From mwknutdesau Where alltrim(TNP_codadmision )+transf(id,"999999");
	 	not in (select alltrim(PAC_codadmision )+transf(TNC_idIngr,"999999") from mwkingproh ) ;
		into cursor mwknutdesa01b

	Select * from mwknutdesa01b left join mwknutDPnova on (TND_codPrest = TNDP_codPrest  ;
		and TNDP_idIngr = mwknutdesa01b.id ) into cursor mwknutsdpedad

	select * from 	mwknutsdpedad ;
	...
	Select mwknutdesa01c.*,mwktotgr.* ;
		...
		group By mwknutdesa01c.TNP_codadmision,TN_CodAgr,tn_ingrediente Into Cursor mwkdesayunos

	If Used('auxidesa')
		Use In auxidesa
	Endif
	select *,id_a as id from mwkdesayunos Into Cursor mwkdesayuno
