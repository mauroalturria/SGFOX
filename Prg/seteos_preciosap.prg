...
If Empty(miparam) AND !INLIST(myip,'172.16.1.7','172.16.120.239')
...
If mresplog = 0
	...
	ldFecha = Date()
	Public fechahora,MCON3
	DO sp_desconexion
		create cursor mwkexe (nomexe c(20),versionactual c(20), launcher c(50),versionminima c(20),idexe n (2))
	insert into mwkexe values ("PRECIOSAP","1.0.0","\\172.16.5.46//C://mail_turnos.exe","1.0.0",23)

*-- Marco el inicio del programa
	Strtofile( Dtoc( ldFecha )+" "+Time()+" - Inicio Programa "+Chr(13)+Chr(10),"PrestaPrecios.txt",1)
	on error Do log_errores With ERROR(), MESSAGE(), MESSAGE(1), PROGRAM(), LINENO()
