...
if lred = 0
	...
	select mwkhistICb.*,medic.matriculas as matic;
		from mwkhistICb ;
		INNER join mwkmedicointall as medic on medic.id = idcodmed;
		order by EIC_fechaHora  ;
		into cursor mwkhistIC

	mret = sqlexec(mcon1, "SELECT pacientes.Pac_codhci,TabintHCE.*,TabintEvol.*,TabintEvolmed.id"+;
...
EIP_evol , EIP_fechaH , EIP_idevol , EIP_tipoEvol , EIP_tipoUsuario , EIP_usuario FROM SQLUser . TabIntEvolParcial

	mret = sqlexec(mcon1, "select EIP_evol , EIP_fechaH , EIP_idevol , EIP_tipoEvol , EIP_tipoUsuario , EIP_usuario "+;
		" from TabIntEvolParcial "+;
		" inner join TabintHCE on EIN_idevol = TabintHCE.id "+;
		" inner join pacientes on pacientes.pac_codadmision = tabintHCE.IH_admision " + ;
		" inner join tabusuario on tabusuario.id = TabIntEvolParcial.EIP_usuario " + ;
		" where pac_codadmision = ?madmis and IH_secuencia = ?msec  &mfiltroNur order by EIN_fechaH "  , "mwkevolNur")
	if mret < 0
		=aerr(eros)
