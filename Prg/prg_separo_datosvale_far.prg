...
**		SP1_SOLIC_SP1_DSOLIC_SP1_nrobono_SP1_Operadro_SP1_sec_SP1_codpun_SP1_codent_SP1_fecNac
	...
	dat_vale(23) = subs(mresp1,  mconta21 + 1, (mconta22 -1) - (mconta21))	&&Operadro
...
do while mconta1 > 0 
	...
	minsu = item_vale(m_i,1)
	item_vale(m_i,3) = subs(mresp2,  mconta2 + 1, (mconta3 -1) - (mconta2))
	if !empty(minsu)
*!*			mret = sqlexec(mcon1, "select INS_descriinsumo " + ;
*!*				" from insumos where INS_codinsumo = ?minsu " , "mwkvalinsu")

		nroitem = nroitem +1
*!*			item_vale(m_i,3) = mwkvalinsu.INS_descriinsumo
	endif
