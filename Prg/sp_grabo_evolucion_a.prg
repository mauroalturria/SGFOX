...
* TxtEdtEvol  anterior
	...
	with mopage3.pgnurse.page1
		...
		mNurse = mNurse + iif(mparTensSis # 0, 'T.Art.Sistòlica mmHg:' + transf(mparTensSis ,'999');
			 + " T.Art.Diastolica mmHg:"+ transf(mparTensDia ,'999') +chr(10),'')
		mparFreCard	= .TxtFcard.value
		mNurse = mNurse + iif(mparFreCard# 0, 'Frecuencia Cardiaca (latidos x min.):'+ transf(mparFreCard,'999') + chr(10),'')
		...
		mNurse = mNurse + iif(mparSatur # 0, 'Saturacion de O2 en sangre %'+ transf(mparSatur ,'999') + chr(10),'')
...
mNurse = iif(!empty(mNurse), 'C.C.V. ' + chr(10) + mNurse,'')
	with mopage3.pgnurse.page2
		...
		mparAdmF = ''
		lhayAF = .f.
		miNurse =  " Administracion de Farmacos "+chr(10)
		for i = 1 to 11
