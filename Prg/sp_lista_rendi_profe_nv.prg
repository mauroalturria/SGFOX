...
mret = sqlexec(mcon1, " select codmed, diasem, fecvigend,medpresta.codesp,codprest  "+;
	...
	", hhmmDes, hhmmHas,demanda,nombre ,generaagen ,codserv " +;
	" from medpresta ,prestadores "+;
	" where codmed = prestadores.id  &mbusco1 "+;
...
mret = sqlexec(mcon1, "select turnos.id,afiliado, fechatur, horatur, codmed, turnos.codesp, " + ;
	...
	"from turnos " + ;
	" where fechatur >= ?mfecdes and fechatur <= ?mfechas  and turnos.codserv<>7000 " +;
	mbusco3 , "mwktodosc1")
if mret < 0
	=aerr(eros)
...
if mfecdes <= mfechalimite
	mret = sqlexec(mcon1, "select turnos.id,afiliado, fechatur, horatur, codmed, turnos.codesp, " + ;
		...
		"where turnos.fechatur >= ?mfecdes and turnos.fechatur <= ?mfechas  and turnos.codserv<>7000 " + ;
		mbusco3 , "mwktodosc2")
	...
	mret = sqlexec(mcon1, "select turnos.id,afiliado, fechatur, horatur, codmed, turnos.codesp, " + ;
		...
		"where afiliado <= 1  and turnos.codserv<>7000 " + ;
	...
	if reccount('mwktodosc1')>0
		select *,iif(afiliado <= 1,id, afiliado) as afi from mwktodosc1 where tipoturno<8 ;
			union all ;
	...
	if reccount('mwktodosl1')>0
		select *,id as afi from mwktodosl1 where tipoturno<8 ;
			union all ;
...
select mwktodosl.id,mwktodosl.afiliado, mwktodosl.fechatur, mwktodosl.horatur;
	...
	group by afi,mwktodosl.codmed, Mwkmedpre.codesp, fechatur, horatur, tipoturno ;
...
select 306 as codmed, "MEDICO DEMANDA ESPONTANEA" as nombre, fechatur,ESP_descripcion, sum(round(((hhasta1 - hdesde1)*porc/100 /3600), 2)) as horas;
	,codserv, codesp ;
	from mwktodosaD1 ;
	group by codesp ;
	order by codesp ;
...
select ESPdescripcion as ESP_descripcion,mwktodosaD.horas as horas, 2200 as codserv;
	...
	sum(0) as consulta, sum(0) as practica, sum(0) as libres, sum(0) as pe,306 as codmed ;
