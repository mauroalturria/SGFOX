...
select habitac,pac_nombrepaciente, hanam,pac_descripdiagn, anios;
		...
		and !(mhora>1100 and mhora<1300 and ctot(pac_fechaadmision) <= horalimite) ;
		,TNP_CodFact1,;
		iif(!empty(TNP_Observaciones1) and hhmm>horacambio ,alltrim(TNP_CodFact1)+" ||| ","")+;
		substr(TNP_Observaciones1,at("|||",TNP_Observaciones1)+4,200))  as TNP_Observaciones1 ;
		,iif(mhora<1300 and isnull(pac_fechaalta),limpia(TNP_CodFact1,horalimite,1),'') as TNP_CodFact1;
		,iif(empty(TNP_Observaciones2) and isnull(pac_fechaalta),TNP_CodFact2,;
		iif(!empty(TNP_Observaciones2) and hhmm>horacambio ,alltrim(TNP_CodFact2)+" ||| ","")+;
		substr(TNP_Observaciones2,at("|||",TNP_Observaciones2)+4,200))  as TNP_Observaciones2 ;
		,iif(isnull(pac_fechaalta),limpia(TNP_CodFact2,horalimite,2),'') as TNP_CodFact2;
		,observaciones,afi_codentidad, pac_codadmision ,0 as lcambio ,pac_fechaalta;
		from mwkpndieta1 into cursor mwkactualiza
endif
