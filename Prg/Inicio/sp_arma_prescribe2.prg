...
if !used("mwkPrescribe")
	create cursor mwkPrescribe (fecha t,medico C(30),Tipo n(1), estado C(1), Leyenda C(100), Informe M, FecHorBlan t null, MedicoBlan C(30) null, TipoM C(1), Guia I null , Baxter C(10) null, Insumo C(11), CodAdmision C(10), IdEvol I,inicio t )
endif
...
endscan
select mwkPrescribe.*,mwkusumed.MATRICULAS as MATRICULA  from mwkPrescribe left join mwkusumed on medico = idusuario into cursor mwkPrescribeaux
select fecha,medico,Tipo,estado,Leyenda,Informe,FecHorBlan;
	,MedicoBlan,TipoM,Guia,Baxter,Insumo,CodAdmision,IdEvol,inicio,nvl(matricula,0) as matriculas ;
	from mwkPrescribeaux into cursor mwkPrescribea
use in select('mwkPrescribe')
use dbf('mwkPrescribea') in 0 again alias mwkPrescribe
