****
** busco paciente internados aislados
****
...
mret = SQLExec(mcon1, "select apv_codadmision, apv_estado , apv_fechaauditoria"+;
	...
	" group by autprevias.id,IAP_TipoAisla,IAP_TipoGermen  ", "mwkpac1aislados")
If mret<1
	Do log_errores With Error(), Message(), Message(1), Program(), Lineno()
	Messagebox("ERROR EN LA GENERACION DEL CURSOR MWKPACINT00",48,"VALIDACION")
	Return .F.
Endif
Select  mwkpac1aislados.*,Strtran(mwktipoaisla.Descrip,"AISLAMIENTO ","") As tipoaisla From mwkpac1aislados,mwktipoaisla  ;
	WHERE GA_TipoAislamiento = mwktipoaisla.estado  Into Cursor mwkpac1aislanew
 
Select mwkpac1aislanew.*,Ctot(Dtoc(apv_fechaauditoria)+" "+Left(Ttoc(apv_horaauditoria,2),5)) As horaaud ;
	,chartipo(apv_codadmision ) As tiponaislanew,nuntipo(apv_codadmision ) As codaisla;
	From mwkpac1aislanew;
	Group By apv_codadmision,apv_estado  Into Cursor mwkpac1aisla
 

FUNCTION nuntipo(mcadmision)
Select Distinc iap_TipoAisla,apv_codadmision,apv_estado From mwkpac1aislanew  WHERE apv_codadmision =mcadmision ;
...
Select Distinc tipoaisla, apv_codadmision,apv_estado   From mwkpac1aislanew WHERE apv_codadmision =mcadmision ;
