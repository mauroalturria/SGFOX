...
mret = sqlexec(mcon1, "select codmed, diasem, fecvigend, fecvigenh, horadesde, horahasta, " + ;
						"tipoturno as pe, abrevio from franjahoraria, tabtipofranja " + ;
...
select mwktodos.diasem,	mwktodos.codmed, afiliado, fechatur, horatur, nombre, ;
	mwktodos.tipoturno, confirmado, hdesde1, hhasta1, cantidad, porcentaje, abrevio, pe, ;
	mwkmedpre.horadesde, mwkmedpre.horahasta ; 
	from mwktodos, mwkmedpre, mwkfranjas ;
	where mwktodos.fechatur >= mwkmedpre.fecvigend and ;
		  ...
		  mwktodos.fechatur >= mwkfranjas.fecvigend and ;
		  mwktodos.fechatur <  mwkfranjas.fecvigenh and ;
		  round(val(strtran(left(ttoc(horatur,2), 5), ':', '')), 0) >= ;
		  round(val(strtran(left(ttoc(mwkmedpre.horadesde,2), 5), ':', '')), 0) and ;
		  round(val(strtran(left(ttoc(horatur,2), 5), ':', '')), 0) <  ;
		  round(val(strtran(left(ttoc(mwkmedpre.horahasta,2), 5), ':', '')), 0) and ;
...
if mlista = 1
	select nombre, fechatur, ;
		...
		sum(iif(afiliado = 0, 1, 0000)) as libre, ;
 		...
 		count(afiliado) as ofrecido, hdesde1, hhasta1, cantidad, porcentaje, abrevio, pe ;
 		from mwktodosa ;
