...
mret = sqlexec(mcon1, "select PAC_nombrepaciente, PAC_edad, sec_codsector, " + ;
	...
	" TabProtQuir.* , Tabquiroflog . FechaMod , Tabquiroflog . UsuarioQ ,"+;
	" lug_pacientes,lug_fechaingreso,lug_horaingreso  " + ;
...
select *,COUNT(pac_codadmision) as cant from mwkpacint01 GROUP BY pac_codadmision ;
 union select *,COUNT(pac_codadmision) as cant from mwkpacint02 GROUP BY pac_codadmision ;
...
select PAC_nombrepaciente, iif(nvl(FechaHoraQuir,ctot("01/01/1900"))<mfechahora,hour(PAC_horaadmision)*100+minute(PAC_horaadmision);
	...
	,  FechaQuirof , HoraEst,HoraEstDesp, HoraFin,HoraIngre, HoraIngquirof ,HoraIndAnes ,HoraFinAnes ,HoraEgre,HoraInic, MateComen  ;
	,NroQuirofano,Nroregistrac, OperCod,codmed,Anestesista,Instrumen,idquirofano,quirofano,laboratorio,AnestesiaTipo,MatInstancia,lug_fechaingreso,lug_horaingreso, ;
	mfechahora - ctot(dtoc(lug_fechaingreso) + '  '  + substr(ttoc(lug_horaingreso),12)) as diferencia,cant ;
	from mwkpacint0 ;
