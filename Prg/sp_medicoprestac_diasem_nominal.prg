...
mret=sqlexec(mcon1," SELECT a.*,b.nombre " +;
	...
	" WHERE &mccpoamb (fecpasivap = ?fechatop or fecpasivap > ?mdmasx ) and a.diasem=?mddiasem AND "+;
...
*" ( SELECT id FROM Prestadores WHERE (fecpasiva >?mdmasX OR fecpasiva=?fechatop) " +;
*" AND (bloquedesde > ?mdmasX or bloquehasta > ?mdmasX) OR (bloquedesde < ?mdmasX and bloquehasta < ?mdmasX)) " +;
*
