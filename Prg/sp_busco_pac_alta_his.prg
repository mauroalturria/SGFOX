...
mret = SQLExec(mcon1, "select PAC_codadmision, PAC_nombrepaciente, PAC_fechaadmision, " + ;
	" PAC_fechaalta, ENT_descrient,ENT_nroprestadorexterno,PAC_sectorinternac, mte_descripcion, ltrim({fn space(1)}),PAC_sexo, " + ;
	...
	" reg_distrito,reg_domicilio,REG_localidad,REG_provincia, REG_telefonos, ENT_codent,PAC_fecnacimiento,REG_nroregistrac , TPV_Estado, " + ;
	" PAC_ordeninternac,ORICodAutoriz,ORINroOrden, ORIObservac, ORITipoOrden, ORIVigDesde,Tabpacobito.PO_FechaIngreso, " + ;
	" pac_fotocrechab,pac_fotocdni,pac_fotoccarnetos, Tabpacobito.PO_admision,Tabpacobito.po_estado,"+;
	" PO_SolicAmbu,coberturas.cob_codentidad, REG_numdocumento ,reg_email,pac_centromedico  " +;
...
*
Select Distinct PAC_codhci From mwkpacalta1a Into Cursor mwkvarios
If Reccount('mwkvarios')=1
	morden = ' order by PAC_codadmision '
Else
	morden = ' order by PAC_nombrepaciente,PAC_codadmision '
Endif
Use In Select('mwkvarios')
Select PAC_codadmision, PAC_nombrepaciente, PAC_fechaadmision,;
	PAC_fechaalta, Ttoc(PAC_horaalta,2) As hora, ENT_descrient,PAC_sectorinternac, mte_descripcion,;
	...
	reg_distrito,reg_domicilio,REG_localidad,REG_provincia, REG_telefonos, ENT_codent,PAC_fecnacimiento ,REG_nroregistrac, ;
	iif(PAC_edad>0,Transf(PAC_edad,"999"),Transf(Round((pac_fechaalta -PAC_fecnacimiento)/30,0),"99")+"M") As anios ;
	, descripegr,tabusuario1.idusuario As operadmi,PO_FechaIngreso,ENT_nroprestadorexterno ;
	,ORICodAutoriz,ORINroOrden, ORIObservac, ORITipoOrden, ORIVigDesde,Nvl(po_estado ,0) As denuncia;
	,Ctot(Dtoc(PAC_fechaaltA)+" "+Ttoc(PAC_horaalta,2)) As fechoraAlta,PO_SolicAmbu,cob_codentidad,REG_numdocumento,reg_email;
	,NVL(pac_centromedico,1) as pac_centromedico ;
	,pac_ordeninternac,pac_fotocrechab,pac_fotocdni,pac_fotoccarnetos,PAC_codhci ;
	from mwkpacalta1a ;
	...
	&mwhere  Group By PAC_codadmision &morden;
	into Cursor &mnomcur
