...
DO CASE 
CASE  mopcion = 1
	mret = sqlexec(mcon1, 'select id, mensaje,fecbaja,turnos,web from tabmensaje ' + ;
		mwhere +  mbusco +mccpoamb+' and ( fecbaja = ?mfecha2 or fecbaja >= ?mfechaVig )  ' , 'mwkmensaje')
CASE mopcion = 2
	mret = sqlexec(mcon1, 'select tabmensaje.id, nombre as nombre, mensaje,tabmensaje.fecbaja'+;
	...
	mret = sqlexec(mcon1, 'select id, pre_descriprest as nombre, mensaje,tabmensaje.fecbaja,turnos,web ' + ;
	...
	mret = sqlexec(mcon1, 'select tabmensaje.id, esp_descripcion as nombre, mensaje,tabmensaje.fecbaja,turnos,web ' + ;
	...
	mret = sqlexec(mcon1, 'select tabmensaje.id, ser_descripserv as nombre, mensaje,tabmensaje.fecbaja,turnos,web ' + ;
		',codmed ,codprest,codespm,codservm,tabmensaje.fecbaja as fpasiva '+;
