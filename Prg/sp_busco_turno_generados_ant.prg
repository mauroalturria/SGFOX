...
parameters mcodmed, mfecdes, mfechas,mbusca,mnoagrupa
if type ('mbusca')#"C"
	mbusca = ''
endif
...
mret = SQLExec(mcon1, 'select turnos.id, fechatur, horatur, codreserva, pre_descriprest, ' + ;
	...
	'medpresta.reservados,canturnos ' + ;
	'from medpresta , turnos left outer join prestacions on ' + ;
	...
	'fechatur >= ?mfecdes and fechatur <= ?mfechas ' + mbusca +;
