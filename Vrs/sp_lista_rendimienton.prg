...
parameter mfecdes, mfechas, mbusco1, mlista, mbusco, mbusco2, mbusco3,mbusco2f,mpe,mbusco3t
...
mret = sqlexec(mcon1, " select codmed, diasem, fecvigend,medpresta.codesp,codserv "+;
	", fecvigenh,  hdesde1, hhasta1, horadesde, horahasta "+;
	", hhmmDes, hhmmHas,demanda,nombre ,generaagen " +;
	" from medpresta,prestadores "+;
	" where diasem > 0 &mbusco1 and codmed = prestadores.id "+;
	" and fecvigenh > ?mfecdes and fecvigend <= ?mfechas "+;
	" and fecvigenh <> fecvigend and codserv<>7000 "+;
...
mret =	sqlexec(mcon1, "select pre_codprest,pre_especialidad,PRE_agendaturnos " + ;
	",ESP_codesp, ESP_descripcion,ESP_cantsinturno  "+ ;
...
select * from Mwkmedpre1 ;
	union select * from Mwkmedpre3 into cursor Mwkmedpreini

select * from Mwkmedpreini,mwkpres ;
	where ESP_codesp = codesp into cursor Mwkmedpre
...
mret = sqlexec(mcon1, "select turnos.id,afiliado, fechatur, horatur, codmed, turnos.codesp, " + ;
	...
	", turnos.codserv,especialid.ESP_descripcion ,ESP_cantsinturno,nombre " + ;
	"from turnos, especialid,prestadores " + ;
	"where turnos.codesp = trim(especialid.ESP_codesp) and tipoturno<9 and prestadores.id = turnos.codmed " + ;
	" and turnos.fechatur >= ?mfecdes and turnos.fechatur <= ?mfechas and turnos.codserv<>7000 " + ;
	+ mbusco3 , "mwktodostom")

mret = sqlexec(mcon1, "select turnos.id,afiliado, fechatur, horatur, codmed, " + ;
	"tipoturno, confirmado, diasem,hhmmtur " + ;
	"from turnos  " + ;
	"where fechatur >= ?mfecdes and fechatur <= ?mfechas and tipoturno<9 and afiliado<2 " + ;
	+ mbusco3 , "mwktodosc10")
...
select mwktodosc10.id,id as afiliado, fechatur, horatur,mwktodosc10.codmed,;
	Mwkmedpre.codesp,tipoturno, confirmado, mwktodosc10.diasem,hhmmtur;
	, Mwkmedpre.codserv,ESP_descripcion ,ESP_cantsinturno,nombre ;
	from mwktodosc10 ,Mwkmedpre ;
	where fechatur >= Mwkmedpre.fecvigend and ;
	fechatur <  Mwkmedpre.fecvigenh and ;
	hhmmtur >= Mwkmedpre.hhmmdes and hhmmtur<Mwkmedpre.hhmmhas and ;
	mwktodosc10.codmed = Mwkmedpre.codmed and ;
	mwktodosc10.diasem = Mwkmedpre.diasem ;
	group by afiliado,mwktodosc10.codmed, codesp,fechatur, horatur, tipoturno ;
	into cursor mwktodosclib

select * from mwktodostom ;
	union	select * from mwktodosclib;
	into cursor mwktodosc1

select fechatur, codmed, codserv, codesp, tipoturno from mwktodosc1 ;
	into cursor mwktodosb1

if mfecdes <= mfechalimite
	mret = sqlexec(mcon1, "select turnos.id,afiliado, fechatur, horatur, codmed, nombre,turnos.codesp, " + ;
		"tipoturno, confirmado, turnos.diasem,hhmmtur " + ;
		", turnos.codserv,especialid.ESP_descripcion,ESP_cantsinturno " + ;
	...
	select fechatur, codmed, codserv, codesp, tipoturno from mwktodosc2;
		where afiliado > 0  into cursor mwktodosb2

	if reccount('mwktodosc1')>0
		select * from mwktodosc1 ;
			union all ;
			select * from mwktodosc2 ;
...
if mlista = 1
	select ESP_descripcion, fechatur, horas, codserv, ;
		sum(iif(codserv = 2200 and confirmado = 1  , 1, 0000)) as consulta, ;
		sum(iif(codserv <> 2200 and confirmado = 1 , 1, 0000)) as practica, ;
		sum(iif(tipoturno=7 and confirmado = 1 , 1, 0)) as pe,ESP_cantsinturno  ;
		from mwktodos ;
...
if mpe=2
	mret =sqlexec(mcon1, "select VAL_codservvale " + ;
		...
		" and VAL_codservvale <> 7000 " + mbusco2  , "mwktotval2")
else
	mret =sqlexec(mcon1, "select VAL_codservvale " + ;
		",pia_cantsolicitada,pia_codprest  "+;
		", VAL_circuitoorigen, VAL_fechasolicitud " + ;
