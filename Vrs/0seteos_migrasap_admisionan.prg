...
*!*	Scan
*!*		mhclin = hclin
*!*		mnroreg = hc_pi_ish.nroreg
*!*		mifec = Transform(Year(fdesde)-2000)+Transform(Month(fdesde),"@L 99")+Transform(Day(fdesde),"@L 99")
*!*		mihas = Transform(Year(fhasta)-2000)+Transform(Month(fhasta),"@L 99")+Transform(Day(fhasta),"@L 99")
*!*		Requery('histambgua')
*!*		Select histambgua
*!*		Locate For his_fechaadmision>=mifec
*!*		Do While !Eof() And his_fechaadmision<=mihas
*!*			mnroadm = his_codadmision
*!*			micontt = Alltrim(Transform(his_codcontrato))
*!*			micont = Left(micontt,Len(micontt)-3)
*!*			pac_tipop2 = Iif(micont="17",2,3)
*!*			mnroreg = hc_pi_ish.nroreg
*!*			mcodent  = 0
*!*			MNUMDOC = 0
...
*!*				m1 = Iif( pac_tipop2=2,"X","")
*!*				m2 = Iif( pac_tipop2=3,"X","")
*!*				m3 = Iif( pac_tipop2<2,"X","")
...
*!*				m7 = Alltrim(mnroadm )
*!*				m8 = Alltrim(Transform(hc_pi_ish.fhasta))
*!*				m9 = "00:00:00"
*!*				m10= Iif(pac_tipop2=2,"AMB","GUA")
*!*				m11= ''
*!*				m12= ''
*!*				m13= ''
*!*				m14= ''
...
*!*				m17 = 'E'
*!*				m18 = Alltrim(Transform(hc_pi_ish.fdesde ))
*!*				m19 = "54035"
*!*				m20 = ''
*!*				For I = 1 To 20
...
*!*	*!*			SELECT pacientes
*!*	*!*			Locate For PAC_fechaadmision=hc_pi_ish.fdesde
*!*	*!*			mnroadm = pacientes.pac_codadmision
*!*	*!*			Requery('cober')
*!*	*!*			If Reccount( "cober")>0
*!*	*!*				Select cober
*!*	*!*				Scan
...
*!*	*!*					m6 = Alltrim(Transform(MNUMDOC ))
*!*	*!*					m7 = Alltrim(mnroadm )
*!*	*!*					m8 = Alltrim(Transform(pacientes.PAC_fechaadmision))
*!*	*!*					m9 = Right("0"+Alltrim(Ttoc(pacientes.PAC_horaadmision,2)),8)
*!*	*!*					m10= Alltrim(pacientes.PAC_sectorinternac)
*!*	*!*					m11= "H"&&alltrim(admisiones.pac_motivo)
*!*	*!*					m12= Alltrim(pacientes.PAC_habitacion)
*!*	*!*					m13= Alltrim(pacientes.PAC_cama)
*!*	*!*					m14= Alltrim(pacientes.PAC_categoria)
*!*	*!*					m15 = Alltrim(Transform(mcodent))
*!*	*!*					m16 = Alltrim(Transform(mnroafi))
*!*	*!*					m17 = 'E'
*!*	*!*					m18 = Alltrim(Transform(minifec ))
*!*	*!*					m19 = Alltrim(Transform(pacientes.PAC_operadm))
*!*	*!*					m20 = Alltrim(Transform(pacientes.PAC_operalta))
*!*	*!*					For I = 1 To 20
*!*	*!*						oExcel.Cells(lnRow + K,I).Value = Evaluate("m" + Alltrim(Transform(I)))
*!*	*!*					Next
*!*	*!*					K = K + 1
...
SELECT actualiza
SCAN
mimat = actualiza.matri
mimed= operadorco 
UPDATE servicios_cabecera SET matricula = mimat WHERE  operadorco = mimed
endscan
*!*	Set Step On
UPDATE servicios_cabecera SET operadorco = ''  WHERE  VAL(operadorco)=99999
SELECT prestacion
SCAN
mimat = prestacion.pre_automatica
mipres = prestacion.pre_codprest
UPDATE valestot SET autom = mimat WHERE  codprest= mipres
endscan
*!*	Set Step On
UPDATE servicios_cabecera SET operadorco = ''  WHERE  VAL(operadorco)=99999
...
mesp = especialid
DO WHILE !EOF()
DO WHILE micuenta = codadmisio AND !EOF()
	IF mivale = codvaleasi 
		replace servsol WITH mesp
	ELSE 
		IF EMPTY(tipopac_in)
			replace servsol WITH mesp
			mivale = codvaleasi
...
Select  admisiones
Set Filter To tipopac_in="X"
Scan
	mnroreg = admisiones.nro_hcli
...
Scan
	...
	If Reccount( "vales_realprest")>0
		Select * FROM vales_realprest WHERE BETWEEN(VAL_fechasolicitud,mfdes,mfhas) INTO CURSOR vales_realpresta
		SELECT vales_realpresta
		Go Top
		mivale = 0
		Do While !Eof()
*	1				2			3			4			5				6			7				;
...
PISO	NUMERO	DESCRIP	URGENCIASERV	CODSERVVALE	PRESTADOR	OPERADORCARGA	OPERADORCONFORME
			mivale = vales_realpresta.VAL_codvaleasist
			m1 = admisiones.tipopac_am
			m2 = admisiones.tipopac_gu
			m3 = admisiones.tipopac_in
			m4 = Alltrim(mnroadm )
			m5 = Alltrim(Transform(vales_realpresta.VAL_codvaleasist))
			m6 = Alltrim(Transform(vales_realpresta.VAL_fechasolicitud))
			m7 = Right("0"+Alltrim(Ttoc(vales_realpresta.VAL_FHSolicitud,2)),8)
			m8 = ''
			m9 = ''
			m10= ''
			m11= Iif(Val(vales_realpresta.VAL_urgenciaserv)=1,"U","")
			m12= Alltrim(Transform(vales_realpresta.VAL_codservvale))
			m13=  Alltrim(Transform(Nvl(vales_realpresta.VAL_prestador,306)))
			m14= Alltrim(Transform(vales_realpresta.VAL_OperadorCarga))
			m15 =  Alltrim(Transform(vales_realpresta.VAL_OperadorConforme))
			For I = 1 To 15
				oExcel.Cells(lnRow + K,I).Value = Evaluate("m" + Alltrim(Transform(I)))
			Next
			K = K + 1
			Do While !Eof() And mivale = vales_realpresta.VAL_codvaleasist
				mivale = vales_realpresta.VAL_codvaleasist
*						CODVALEASIST	CODPREST	CANTSOLICITADA	ESPECIALIDAD

				m1 = Alltrim(Transform(vales_realpresta.VAL_codvaleasist))
				m2 =  Alltrim(Transform(vales_realpresta.PRE_codprest))
				m3 = Alltrim(Transform(vales_realpresta.PIA_cantsolicitada))
				m4 = Alltrim(Transform(vales_realpresta.PRE_especialidad))
				For I = 1 To 4
					oExceld.Cells(lnRow + l,I).Value = Evaluate("m" + Alltrim(Transform(I)))
				Next
				L = L + 1
				Skip
			Enddo
