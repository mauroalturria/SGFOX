*!*	SELECT cuit
*!*	SCAN
*!*	micuit = TRANSFORM(matri,"@L 99-99999999-9")
*!*	micod = codigo
*!*	UPDATE prestasap SET cuit= micuit WHERE codigo = micod
*!*	endscan
...
lcDir = "c:\SAP\"
lcArc = "sap_medicos.txt"
lbProcesar = .T.
mnarch = Fcreate(lcArc)
USE IN SELECT('PRESTA')
USE IN SELECT('PRESTAA')
...
*!*	Select * From prestadores Where Id>1 And Nvl(tpf_filtro,6)=6 And ;
*!*	(fecpasivap=Ctod("01/01/1900") Or fecpasivap>=Ctod("01/10/2020")) AND ;
*!*	val(cuil)>10 AND cuil#'27-32359666-8' AND cuil NOT in (SELECT cuit FROM prestasap)  Into Cursor presta

 Select * From nuevocuil Where Id>1 And INLIST(Nvl(tpf_filtro,7)=7   ;
  Into Cursor presta
...
Scan All
	If Val(Nvl(MWKPRESTA.cuil,''))>10
		...
		m14 = IIF( MWKPRESTA.codpcia >24,1,MWKPRESTA.codpcia) &&Provincia(MWKPRESTA.codpcia)
