...
*mret = SQLExec(mcon1,"select * FROM TabMedFoto where  FechaBaja = ?mfecpas and tipo = 2 and idmedico in (5463) order by id desc ","mprofFotList")
 mret = SQLExec(mcon1,"select IdMedico FROM TabMedFoto where  tipo =3  ","mprofFotoctr")
...
Select * From mprofFotoctr Where idmedico   In (Select id From prestadores ) Into Cursor mprofFotList
...
Scan All
	...
	mifirma = "c:\documenta\"+Alltrim(Transform(mprofFotList.idmedico))+"_firma_ms.tif" 
	prg_carga_firma(mprofFotList.idmedico,2,.t.)
	COPY file  "C:\temp\imagenes\firmas\firma_ms.tif" TO &mifirma 
...
*!*				lcnombre     = lcdirectorio  +'\' + Alltrim(Str(mwkprestador.Id))  + '.tif'
