...
mret = sqlexec(mcon3, "select hab_codhabitacion, hab_codcama,PAC_nombrepaciente " + ;
...
If mrepro = 1
	mret = sqlexec(mcon3, "select TabNutPaciente.* " + ;
		" from pacinternad left join pacientes on pin_codadmision = PAC_codadmision " + ;
		" left join TabNutPaciente on pin_codadmision = TNP_codadmision " + ;
	...
	mret = sqlexec(mcon3, "delete from TabNutPaciente " + ;
	...
	mret = sqlexec(mcon3, "select TNP_codadmision " + ;
		" from TabNutPaciente  " + ;
...
Select Distinct TNP_codadmision From mwkdietdes Into Cursor mwkctrl

*** Nutricion anterior
mret = sqlexec(mcon3, "select *  from TabNutPaciente " + ;
...
If mrepro = 1 Or Reccount("mwkdietdes") < Reccount("mwkctrl")
	...
	** ahora vemos compatibilidades
	...
	mfecpas = Ctod("01/01/1900")
	** indispensables
	mret = sqlexec(mcon3, "select  TND_codPrest,TNP_codadmision, TND_idPaciente ,TabNutIngr.*,PAC_edad  " + ;
		" from pacinternad left join pacientes on pin_codadmision = PAC_codadmision " + ;
		" left join TabNutPaciente on pin_codadmision = TNP_codadmision " + ;
		" left join TabNutDetalle on TND_idPaciente = TabNutPaciente.id" + ;
	...
	** no compatibles
	...
	If mret<1
		...
		Messagebox (eros(3))
	Endif
	mret = sqlexec(mcon3, "select  TabNutComp.* " + ;
		" from TabNutIngr " + ;
	...
	If mret<1
		...
		Messagebox (eros(3))
	Endif
	mret = sqlexec(mcon3, "select  TabNutDietaPrest.* " + ;
...
&&&  Genero los registros
	...
	Do While !Eof('mwkdesayuno')
		...
		If lsigue
			...
			mret =sqlexec(mcon3, "select id from TabNutPaciente "+;
...
do sp_busco_dieta with mfecha,mtiposer
